...
_mount_chroot_fs() {
    ...
    if [ -n "$options" ]; then
        mount -t "$fs_type" -o "$options" "$source" "$target" 2>/dev/null
    else
        mount -t "$fs_type" "$source" "$target" 2>/dev/null
    fi
    if [ "$?" -ne 0 ]; then
...
cleanup_chroot_test_root() {
    for target in /mnt/work/tgoskits/target /mnt/run /mnt/dev/shm /mnt/dev /mnt/sys /mnt/proc; do
...
if ! grep -q ' /mnt ' /proc/mounts 2>/dev/null; then
    mount -t ext4 /dev/vd0 /mnt 2>/dev/null || \
        mount -t ext4 /dev/vda /mnt 2>/dev/null || true
fi
maybe_enter_shell
setup_dynamic_linker
