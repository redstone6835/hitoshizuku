...
impl PartialEq for PnpId {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (
                Self::Pci { segment: sa, bus: ba, device: da, function: fa },
                Self::Pci { segment: sb, bus: bb, device: db, function: fb },
            ) => sa == sb && ba == bb && da == db && fa == fb,
            (
                Self::Usb { bus_id: bia, address: aa, interface: ia },
                Self::Usb { bus_id: bib, address: ab, interface: ib },
            ) => bia == bib && aa == ab && ia == ib,
            (
                Self::Platform { name: na, identity: ia },
                Self::Platform { name: nb, identity: ib },
            ) => na == nb && ia == ib,
            (
                Self::Dynamic { fingerprint: fa, bus: ba, contract: ca, identity: ia },
                Self::Dynamic { fingerprint: fb, bus: bb, contract: cb, identity: ib },
