...
/// 在常驻 General 侧构造完成类型擦除的 provider registration 资源。
///
/// 动态 ELM 只接收 trait object，不在模块镜像内生成 [`PnpHandleResource`] 的
/// trait vtable。
#[kernel_symbols::export(
    name = "general.dev.dt_provider.provider_pnp_resource_boxed",
    contract = "kernel.general.dt-provider@1",
    version = 1,
    capabilities = kernel_symbols::capability::DEVICE_RESOURCE,
    flags = kernel_symbols::KERNEL_SYMBOL_FLAG_RETURNS_OWNED
)]
pub fn provider_pnp_resource_boxed(
    handle: DtbProviderHandle,
    label: &'static str,
) -> alloc::boxed::Box<dyn PnpResource> {
    alloc::boxed::Box::new(provider_pnp_resource(handle, label))
}
...
/// 在常驻 General 侧构造完成类型擦除的 consumer lease 资源。
///
/// 这保证 [`DtbLeasePnpResource`] 的 trait vtable 和释放实现不会成为动态 EKI 的
/// 裸 Rust 链接依赖。
#[kernel_symbols::export(
    name = "general.dev.dt_provider.lease_pnp_resource_boxed",
    contract = "kernel.general.dt-provider@1",
    version = 1,
    capabilities = kernel_symbols::capability::DEVICE_RESOURCE,
    flags = kernel_symbols::KERNEL_SYMBOL_FLAG_RETURNS_OWNED
)]
pub fn lease_pnp_resource_boxed(
    lease: DtbResourceLease,
    label: &'static str,
) -> alloc::boxed::Box<dyn PnpResource> {
    alloc::boxed::Box::new(lease_pnp_resource(lease, label))
}

/// 直接消费解析器产生的规范化 provider 引用。
#[kernel_symbols::export(
    name = "general.dev.dt_provider.acquire_reference",
    contract = "kernel.general.dt-provider@1",
    version = 1,
    capabilities = kernel_symbols::capability::DEVICE_RESOURCE,
    flags = kernel_symbols::KERNEL_SYMBOL_FLAG_MUTATES_STATE
