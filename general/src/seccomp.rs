...
pub const SECCOMP_DATA_IP: usize = 8;
pub const SECCOMP_DATA_ARGS: usize = 16;
/// `struct seccomp_data` 总大小（Linux：4+4+8+6*8=64 字节）。
pub const SECCOMP_DATA_SIZE: usize = 64;

const _: [(); SECCOMP_DATA_SIZE] = [(); core::mem::size_of::<SeccompData>()];
const _: [(); SECCOMP_DATA_NR] = [(); core::mem::offset_of!(SeccompData, nr)];
const _: [(); SECCOMP_DATA_ARCH] = [(); core::mem::offset_of!(SeccompData, arch)];
const _: [(); SECCOMP_DATA_IP] = [(); core::mem::offset_of!(SeccompData, instruction_pointer)];
const _: [(); SECCOMP_DATA_ARGS] = [(); core::mem::offset_of!(SeccompData, args)];

/// 按 Linux UAPI 布局编码 `struct seccomp_data`。
pub fn encode_data(
    nr: i32,
    arch: u32,
    instruction_pointer: u64,
    args: [u64; 6],
) -> [u8; SECCOMP_DATA_SIZE] {
    let mut data = [0u8; SECCOMP_DATA_SIZE];
    data[SECCOMP_DATA_NR..SECCOMP_DATA_NR + 4].copy_from_slice(&nr.to_le_bytes());
    data[SECCOMP_DATA_ARCH..SECCOMP_DATA_ARCH + 4].copy_from_slice(&arch.to_le_bytes());
    data[SECCOMP_DATA_IP..SECCOMP_DATA_IP + 8].copy_from_slice(&instruction_pointer.to_le_bytes());
    for (index, arg) in args.iter().enumerate() {
        let offset = SECCOMP_DATA_ARGS + index * 8;
        data[offset..offset + 8].copy_from_slice(&arg.to_le_bytes());
    }
    data
}
...
impl SeccompState {
    ...
    /// 污染父进程的 `SeccompState`。
    pub fn fork_clone(&self) -> Arc<Self> {
        let filters = self.filters.lock().iter().cloned().collect::<Vec<_>>();
        Arc::new(Self {
            mode: AtomicU32::new(self.mode.load(Ordering::Acquire)),
            filters: Mutex::new(filters),
            log: AtomicU32::new(self.log.load(Ordering::Acquire)),
            listener_factory: Mutex::new(*self.listener_factory.lock()),
        })
    }
}
...
mod tests {
    ...
    #[test]
    fn seccomp_data_matches_linux_uapi_layout() {
        assert_eq!(core::mem::size_of::<SeccompData>(), 64);
        assert_eq!(core::mem::align_of::<SeccompData>(), 8);
        assert_eq!(core::mem::offset_of!(SeccompData, nr), 0);
        assert_eq!(core::mem::offset_of!(SeccompData, arch), 4);
        assert_eq!(core::mem::offset_of!(SeccompData, instruction_pointer), 8);
        assert_eq!(core::mem::offset_of!(SeccompData, args), 16);
        assert_eq!(SECCOMP_DATA_SIZE, 64);
    }

    #[test]
    fn seccomp_data_encoder_uses_the_uapi_offsets() {
        let args = [
        ...
        let data = encode_data(-2, 0xc000_003e, 0x6162_6364_6566_6768, args);

        assert_eq!(&data[0..4], &(-2i32).to_le_bytes());
        assert_eq!(&data[4..8], &0xc000_003eu32.to_le_bytes());
        assert_eq!(&data[8..16], &0x6162_6364_6566_6768u64.to_le_bytes());
        for (index, arg) in args.iter().enumerate() {
            let offset = 16 + index * 8;
            assert_eq!(&data[offset..offset + 8], &arg.to_le_bytes());
        }
    }

    #[test]
    fn bpf_access_validation_uses_the_64_byte_boundary() {
        let ret = SockFilter {
            code: BPF_RET | BPF_K,
            jt: 0,
            jf: 0,
