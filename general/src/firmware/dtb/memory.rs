...
use allocator::{MemorySegment, PAGE_SIZE};
...
/// 按 NUMA bank 边界切分的物理内存与固件归属描述。
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct DtbNumaMemoryLayout {
    pub usable_segments: Vec<MemorySegment>,
    pub numa_ranges: Vec<DtbNumaMemoryRange>,
}

/// 由 DT `/memory` 节点提供的 NUMA 归属。
///
/// 该描述保留在固件与设备发现层，不改变物理分配器的放置语义。
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct DtbNumaMemoryRange {
...
/// 把最终可用 RAM 按 DT NUMA bank 边界切分，并生成设备发现层使用的归属标签。
///
/// 物理分配器仍按普通 RAM 段处理这些范围；NUMA 放置策略后续可以在不改变固件
/// 解析 ABI 的情况下单独接入。
    ...
    for bank in &description.memory_banks {
        ...
        for &range in &bank.ranges {
            ...
            if let Some(range) = page_normalized_segment(native)? {
                tagged.push(DtbNumaMemoryRange { range, node_id });
            }
        }
    }
    ...
    for segment in usable_segments {
        ...
        for pair in boundaries.windows(2) {
            ...
            if let Some(node_id) = node_id {
                numa_ranges.push(DtbNumaMemoryRange { range, node_id });
            }
        }
    }
