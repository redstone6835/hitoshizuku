...
pub fn kernel_start_init(context: &StartContext) {
    ...
    // ── 阶段 5：完成设备注册后选择根文件系统并准备 /dev ────────────────
    //
    // 设备阶段必须先运行：VirtIO/PCI 驱动只有在 integrated device phase
    // 完成后才会发布块功能，随后 boot_root 才能扫描 GPT/MBR 并解析
    // `root=/dev/vd0p1`。devtmpfs 本身可以先创建，但挂载到最终根目录
    // 要等 root 选择完成，避免把它错误地挂到临时 tmpfs 上。
