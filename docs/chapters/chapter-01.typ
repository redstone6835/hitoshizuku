...
#continued-table(
  ...
  (
    ...
    table.cell(fill: stable-fill)[将建立电源控制和日志能力置于启动链路前端。],
  ),
  kind: "表",
...
#figure(caption: figure-caption("图", "1-1", [项目分层结构]))[
  #layer-card(
    "基础库层",
    [可复用基础设施。包含分配器、文件系统、网络、调度以及设备相关库，不依赖上层启动策略。],
    fill: soft-fill,
  )
  ...
  #layer-card(
    "统一架构接口层",
    [封装 `arch` 模块的具体实现，为 `kernel` 模块提供稳定函数表和运行期钩子。],
    fill: handoff-fill,
  )
  #align(center)[#text(fill: rgb("#2d5f73"))[↑]]
  #layer-card(
    "策略与集成层",
    [消费 `hal` 模块与 `general` 模块，完成内存、设备、VFS、控制台、调度器以及用户态入口的编排。],
    fill: stable-fill,
  )
]

这套分层的关键不在于目录名称，而在于依赖方向。这种单向依赖给启动路径带来两个直接收益：第一，平台差异被限制在 `arch` 模块和少量 `hal` 适配中；第二，平台无关层可以用相同的代码处理 DTB 路径和 ACPI 路径产出的结果。我们在调试启动问题时，也能先判断错误发生在交接之前还是交接之后。这个判断往往比单纯查看日志更重要，因为它直接决定是应当检查汇编入口、固件快照，还是检查通用子系统初始化顺序。
...
#continued-table(
  ...
  (
    ...
    table.cell(
      fill: handoff-fill,
    )[初始化早期日志和启动分配器，复制固件表，选择 DTB 或 ACPI，准备地址转换和分配器回调。],
    table.cell(fill: handoff-fill)[启动上下文。],
    table.cell(fill: soft-fill)[启动初始化],
    table.cell(
      fill: soft-fill,
    )[校验上下文，根据固件来源进入 DTB 或 ACPI 路径，激活内存、设备、VFS 和控制台等各子系统。],
...
根文件系统的选择在 DTB 路径中具有优先级。外部 initramfs 初始内存文件系统或内建 initramfs 初始内存文件系统优先。若不存在 initramfs 初始内存文件系统，再尝试从已经注册的块设备中挂载根盘。ACPI 路径当前采用 tmpfs 临时文件系统作为启动根。两条路径都会挂载 devtmpfs 文件系统到 `/dev`，并挂载 `/dev/shm` 与 `/sys`。需要注意的是，procfs 文件系统在核心文件系统阶段完成注册，但当前启动路径没有在这里自动挂载 `/proc`。两条路径最终都会创建 VFS 上下文、挂载命名空间、根目录和凭据，并通过 `sched::stash_boot_vfs_parts` 函数交给调度层。调度器随后给 PID 1 安装 VFS 上下文和文件描述符表。
...
`start_init_process` 函数负责把 PID 1 替换为真实用户态镜像。其选择顺序与 Linux 一致：initramfs 根先尝试 `rdinit=` 或默认 `/init`，真实根优先执行显式 `init=`，未指定时再依次尝试 `/sbin/init`、`/etc/init`、`/bin/init` 和 `/bin/sh`。第一个可加载的用户镜像会获得用户地址空间、入口地址和初始栈。内核随后构造用户态陷阱帧，并通过架构相关返回路径进入用户态。此后系统进入常规调度阶段，启动上下文不再参与运行期控制流。
...
第四是把启动顺序设计成可解释的依赖链。启动初始化中很多顺序看似只是工程习惯，实际都对应明确约束。BSS 必须先清零再保存启动参数。电源控制必须早于分配器和页表等高风险操作。物理页分配器必须早于虚拟内存管理和 Slab 分配器。devtmpfs 文件系统需要早于设备扫描接收投影事件。控制台切换要晚于字符设备注册，以便早期日志保留尽可能久。我们把这些顺序固定在启动控制流中，并通过注释、伪代码和上下文校验说明原因。这样做的价值在调试阶段尤为明显。一次启动失败可以沿依赖链向前定位，而不是在多个全局状态之间反复猜测。
