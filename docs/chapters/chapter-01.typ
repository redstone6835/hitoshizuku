...
我们在设计内核启动路径时面对的首要问题，是同一套内核需要同时运行在 LoongArch64 架构和 RISC-V64 架构上。两种平台的差异并不只体现在指令编码和寄存器名称上。固件交接方式不同，早期地址空间不同，内存描述来源不同，设备发现路径也不同。RISC-V64 架构的 QEMU 虚拟机直启路径由启动寄存器传入设备树二进制（Device Tree Blob, DTB）物理地址。LoongArch64 架构的启动应用二进制接口（Application Binary Interface, ABI）则传入可扩展固件接口（Extensible Firmware Interface, EFI）标记、命令行地址和 EFI 系统表地址。当前 LoongArch64 架构加载器以 EFI 系统表为固件发现入口，优先从 EFI 配置表选择高级配置与电源接口（Advanced Configuration and Power Interface, ACPI），缺省回退到扁平设备树（Flattened Device Tree, FDT）。若平台无关层直接理解这些细节，启动代码会很快形成大量分支。若架构层只交出少量原始指针，后续子系统又无法判断这些指针的生命周期和有效性。
...
跨架构启动代码的难点不在于写出两个入口。难点在于确定哪些差异应当留在架构层，哪些信息应当向上交给通用内核。启动早期没有完整堆分配器，没有统一设备模型，也没有稳定的日志输出。此时任何复杂依赖都可能放大故障面。启动后期又必须准备运行期系统。物理页分配器需要可用内存段。设备子系统需要内存映射输入输出（Memory-Mapped I/O, MMIO）地址转换能力。控制台需要固件中的串口描述。调度器需要虚拟文件系统（Virtual File System, VFS）根上下文和标准文件描述符。每一步都依赖前一步已经产出的稳定结果。
...
两个边界在这条链路中尤其重要。第一个边界是早期入口到架构加载器。这里要求代码已经有临时栈，未初始化数据段（Block Started by Symbol, BSS）状态也已知。第二个边界是架构加载器到 `__kernel_start_init` 入口。这里要求固件视图已经稳定，地址转换函数已经可用，内核镜像范围和可选内存图已经填入上下文。后续平台无关层只依赖这些结果。
...
RISC-V64 架构入口还有一个实际约束。当前编译配置下，LLVM 编译器可能在发布模式生成浮点相关指令。我们在进入 Rust 语言代码前设置 `sstatus.FS` 寄存器字段，确保早期 Rust 语言路径不会因为浮点单元（Floating-Point Unit, FPU）状态不可用而陷入异常。这个细节说明早期环境并非只服务于汇编入口，它还要满足编译器生成代码的最低运行条件。
...
RISC-V64 架构的直启路径相对直接。启动寄存器提供 DTB 物理地址，加载器复制 DTB，记录 `StartBootProtocol::Direct` 枚举变体，并将启动内存图设为 `None`。后续 DTB 解析器从 `/memory` 节点取得随机存取内存（Random Access Memory, RAM）信息。LoongArch64 架构的路径更复杂。加载器保存命令行，并以 EFI 系统表为固件发现入口。若配置表中存在根系统描述指针（Root System Description Pointer, RSDP）且 EFI 内存映射可用，它会选择 ACPI 并复制 ACPI 表；否则在存在 FDT 时复制 DTB 视图。启动协议根据入口传入的 EFI 标记记录为 `Efi` 或 `Direct` 枚举变体，但当前实现仍要求能取得可用的 EFI 系统表来发现 ACPI 或 FDT。当前 ACPI 路径要求存在可用的启动内存映射，因为 ACPI 本身描述平台结构，却不提供可直接交给分配器的普通 RAM 列表。
...
ACPI 路径先通过复制后的 RSDP 和表映射建立 ACPI 表视图，再从多中断控制器描述表（MADT）取得 CPU 数量，从串口控制台重定向表（SPCR）和 ACPI 命名空间发现串口设备，从命名空间发现 virtio-mmio 设备，并解析电源控制寄存器。ACPI 路径的普通 RAM 输入来自启动内存区域列表。这与 DTB 路径不同，但在进入分配器之前，两者都会整理出同一种内存段列表。
...
#pseudo-sample("1-5", [主入口控制流], kind: "代码")[
  ...
  fn main() -> ! {
      ...
      run_optional_tests_or_benchmarks();
...
`sched::boot_init` 函数的注册顺序同样由依赖决定。它先通过 `hal` 模块注入上下文切换、时间、陷入栈、内存切换和系统调用相关钩子。随后注册 `fork` 系统调用和 `clone` 系统调用需要的扩展复制钩子，再注册退出钩子和退出前钩子。退出前钩子要早于地址空间释放执行，因为 robust futex 清理机制和 clear-child-tid 机制需要在用户虚拟地址空间仍可访问时处理。接着注入用户进程镜像操作表、虚拟内存切换钩子和任务 CPU 状态发布钩子，最后创建 init 任务。
...
`start_init_process` 函数负责把 PID 1 替换为真实用户态镜像。它依次尝试 `/init`、`/sbin/init` 和 `/bin/init`。第一个可加载的用户镜像会获得用户地址空间、入口地址和初始栈。内核随后构造用户态陷入帧，并通过架构相关返回路径进入用户态。此后系统进入常规调度阶段，启动上下文不再参与运行期控制流。
