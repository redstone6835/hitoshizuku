...
并发与同步机制贯穿整个内核。它不像 VFS 或网络那样有单一用户可见接口，却决定了所有接口能否可靠运行。本章把同步机制分成短临界区、可睡眠临界区和事件等待三类，并说明它们与调度器状态机的关系。这些机制共同支撑了内核在高并发负载下的稳定性。同步机制的价值来自每个原语清晰的上下文限制和交接规则，也来自这些规则在不同子系统中的一致复用。只要这些规则保持稳定，后续扩展新的套接字等待、设备事件或文件系统缓存时，就可以沿用同一套并发模型。同步层因此成为连接调度、资源管理和用户态可观察行为的一条基础线索。它把局部互斥、条件等待和生命周期收束组织在同一套工程纪律之下，使复杂路径仍然能够被逐段推理，也使故障定位能够从任务状态、等待条件和锁边界三个方向同时展开。这也是后续章节能够减少重复解释的重要原因之一，并能提升审查效率。实际维护时，开发者只要先确认当前路径所在上下文，再确认业务条件由谁保护，最后确认唤醒是否在锁外发生，就能排除大部分同步设计错误。
