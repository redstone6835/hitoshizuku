...
#continued-table(
  ...
  (
    ...
    table.cell(
      fill: stable-fill,
    )[文件系统实现索引节点操作集（`InodeOps`）与文件操作集（`FileOps`），设备节点通过投影器注入节点载荷。],
...
#figure(caption: figure-caption("图", "4-1", [VFS 的单向信息流]))[
  #layer-card(
    "POSIX 系统调用层",
    [`openat`、`read`、`write`、`close`、`stat`、`mount`、`ioctl`、`mmap`],
    fill: soft-fill,
  )
  ...
  #layer-card(
    "文件系统与投影层",
    [tmpfs 内存文件系统、devtmpfs 文件系统、procfs 文件系统、sysfs 文件系统、pipefs 管道文件系统和设备文件投影器],
    fill: warm-fill,
  )
...
VFS 与设备模型的交接发生在 devtmpfs 文件系统和设备文件投影器。设备模型发布设备能力。投影层生成设备节点规范。devtmpfs 文件系统创建节点并在打开时构造文件操作集。这个过程保持单向。设备移除时，设备能力先进入失效状态。已打开文件的后续 I/O 会在适配器中看到设备不可用。新打开则会被节点状态或适配器拒绝。这样可以保证热插拔时用户态名字空间和底层设备生命周期不会互相撕裂。
