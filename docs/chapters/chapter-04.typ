...
第二是缓存和回收围绕引用关系设计，而不是围绕全局扫描设计。目录项缓存使用分片降低锁竞争。索引节点缓存使用弱引用避免把历史对象永久保活。挂载对象的繁忙检测通过打开计数完成，不扫描所有任务的文件描述符表。`unlink` 后的索引节点先从命名空间消失，再等最后一个强引用释放后调用驱动的驱逐操作。这个设计把大范围遍历转化为局部引用变化，热路径开销更可控。事后回顾，这种引用关系也让 LTP 中大量 `open`、`close`、`rename` 和 `unlink` 组合测试更容易分析，因为每个对象的所有权来源是清楚的。
