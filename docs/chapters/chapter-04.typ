...
第二是能力冻结模型强化了 VFS 的安全边界。打开路径时，VFS 完成路径解析、权限检查、挂载标志检查和文件系统 `open` 操作。成功之后，文件对象保存访问模式和凭据快照。后续读写不再重新解析路径，也不依赖路径当前权限。这个模型避免了检查和使用之间的竞态窗口。它还让文件描述符成为可以安全传递的能力对象。父子进程通过 `fork` 共享同一文件对象时，偏移共享和访问能力共享都来自同一对象。多个描述符通过 `dup` 指向同一文件对象时，描述符级 `CLOEXEC` 与文件级状态也能明确区分。这个设计把安全判断集中在 `open` 边界，热路径因此更短，错误也更容易定位。

第三是把挂载实例作为命名空间策略对象，避免把策略压入文件系统实例。超级块负责文件系统内容和驱动状态。挂载对象负责只读、禁止执行、禁止设备等命名空间策略。挂载命名空间再负责进程看到的挂载集合。这个结构让同一个文件系统可以在不同位置呈现不同策略，也让 `pivot_root` 和 `chroot` 可以通过切换命名空间边界实现。我们没有为挂载树引入过度细粒度的锁，而采用单锁保护挂载对象集合和索引。挂载操作低频，路径解析中的查询短暂。这个取舍降低了死锁风险，并使 `pivot_root` 这类跨多个节点的操作能够保持原子。

第四是特殊文件系统被纳入同一对象模型，但保留各自的数据来源。tmpfs 内存文件系统的数据来自内存。procfs 进程文件系统的数据来自运行时状态。devtmpfs 设备文件系统的节点来自设备能力投影。pipefs 管道文件系统的对象来自管道缓冲区。它们都提供索引节点、目录项和文件操作集，因此系统调用层无需关心背后来源。与此同时，devtmpfs 设备文件系统节点打开时仍能恢复类型化设备能力，procfs 进程文件系统读取时仍能动态生成内容，pipefs 管道文件系统仍能维护读端和写端引用计数。统一接口没有抹平内部差异，内部差异也没有突破系统调用边界。这个关系与第三章的设备能力多轨结构保持一致。

第五是缓存和回收围绕引用关系设计，而不是围绕全局扫描设计。目录项缓存使用分片降低锁竞争。索引节点缓存使用弱引用避免把历史对象永久保活。挂载对象的繁忙检测通过打开计数完成，不扫描所有任务的文件描述符表。`unlink` 后的索引节点先从命名空间消失，再等最后一个强引用释放后调用驱动的驱逐操作。这个设计把大范围遍历转化为局部引用变化，热路径开销更可控。事后回顾，这种引用关系也让 LTP 中大量 `open`、`close`、`rename` 和 `unlink` 组合测试更容易分析，因为每个对象的所有权来源是清楚的。

这些创新共同形成了 VFS 的工程价值。它向上提供稳定的 POSIX 兼容层，向下容纳不同文件系统和设备投影，向侧面与进程、内存和设备管理保持清晰交接。路径解析、打开能力、挂载命名空间和对象回收分别处在各自边界内。新增文件系统时，只需要实现驱动接口。新增设备节点时，只需要补充投影器和适配器。新增进程隔离语义时，可以从 VFS 上下文和挂载命名空间扩展。VFS 因此成为用户态观察内核资源的结构化边界，而不只是单个文件系统的包装层。
