...
僵尸状态的价值在于保留 `wait` 可见信息。父任务可能在子任务退出很久以后才调用 `wait`。若 `exit` 立即释放任务对象，退出码、终止信号、资源用量和 procfs 进程文件系统可见状态都会丢失。若僵尸任务继续保留完整虚拟内存、文件表和内核栈，短生命周期进程密集创建和退出时会快速积累内存压力。当前实现把轻量状态留在任务对象本体中，把重量级状态交给幂等的退出钩子清理。这样既满足 `wait` 语义，也控制了僵尸任务的资源占用。
