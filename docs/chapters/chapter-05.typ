...
创建流程可以拆成四步。第一步创建任务对象，建立父子关系、线程组和进程组关系，并在 PID 命名层登记整数名字。第二步复制或共享扩展状态。调度核心不理解 VFS 上下文、文件描述符表和用户虚拟地址空间的内部结构，而是调用内核在启动期注册的任务扩展克隆钩子（`TaskExtCloneHook`）。第三步构造用户上下文。架构层复制或调整陷入帧，处理子栈、TLS 和子线程标识地址。第四步安装内核执行体，把子任务放入运行队列。
...
这种注入方式与第一章的启动上下文和第二章的架构内存接口保持一致。平台相关层负责寄存器布局和汇编切换。平台无关层负责生命周期、状态机和资源关系。这样可以让 RISC-V64 与 LoongArch64 共用任务对象结构和调度逻辑，同时保留各自异常返回、陷入帧和上下文切换的实现差异。
...
第二是把 PID 从内部身份降级为 ABI 名字。PID 对用户程序不可或缺，但它不适合作为内核内部的主引用。整数会复用，查找需要全局表，生命周期也容易与任务对象脱节。我们让内部路径传递任务强引用，PID 登记表只保存弱引用。`getpid`、`kill`、`waitpid` 和 procfs 进程文件系统在边界上把整数 PID 翻译为任务引用。这个设计减少了 PID 表锁对内部路径的影响，也避免登记表保活已经退出的任务。更重要的是，它为 PID 命名空间保留了自然结构。任务可以在多层命名空间中拥有不同 PID，调度核心仍然只处理同一个任务对象。
...
第四是退出和回收被设计为可观察状态与资源释放的分阶段协议。POSIX 要求父进程能够在子进程退出后读取退出码和终止原因，所以僵尸状态必须保留。系统稳定性又要求已经退出的任务尽快释放虚拟内存、文件表和内核栈，尤其是在 `fork` 压力测试中。当前实现把 `wait` 可见的轻量状态留在任务对象中，把重量级资源通过幂等钩子清理，把父子关系保留到 `wait` 回收。父先退出时，子任务移交给 init 任务。这个分阶段协议让 `wait` 语义、资源回收和孤儿收养可以同时成立。

第五是等待队列使用弱引用和双重检查协议。弱引用避免等待队列保活任务，也让取消等待、超时和任务退出后的残留条目可以自然清理。准备等待阶段先登记等待者，再切换睡眠状态，并在真正调度出去前重新检查条件。这个顺序解决了事件发生在检查条件和进入睡眠之间的丢失唤醒问题。唤醒回调在锁外执行，避免等待队列锁和运行队列锁反向嵌套。这个设计在管道、套接字、futex 机制、随机数设备和退出等待队列等路径上重复使用，减少了每个子系统自行处理睡眠竞态的机会。

第六是凭据与信号都采用快照化和分层共享。凭据用不可变对象加整体替换，权限检查不会读到撕裂状态。信号把任务私有待处理队列、任务私有阻塞掩码和线程组共享动作表分开，既能表达线程私有信号，也能表达进程级信号处理方式。这个分层对 POSIX 兼容很关键。一个线程可以独立屏蔽信号，整个线程组又共享处理函数。发送阶段只负责入队和唤醒，真正的信号帧构造推迟到返回用户态前。异步通知因此不会破坏内核关键区。
