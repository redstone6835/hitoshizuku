...
我们把系统调用实现和重启框架分开。系统调用实现遇到可打断等待时返回内部错误码或 `EINTR`。分发器在返回前检查待处理信号，构造信号帧，并根据系统调用上下文和信号动作标志决定是否设置重启状态。重启状态可以通过调整程序计数器、保存原参数或使用 `restart_syscall` 入口来表达。具体策略需要和架构陷阱帧协作。
