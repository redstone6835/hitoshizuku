...
待处理位图用于快速判断是否有信号。`SigInfo` 队列保存发送者 PID、UID、信号码和用户提供的原始负载。标准信号和实时信号在语义上有不同要求，`sigqueueinfo` 还需要保留用户提供的信号信息。当前实现没有把待处理状态简化成单个位图，而是使用队列保存具体 `SigInfo`，位图作为快速索引。取出信号时会跳过被信号屏蔽字屏蔽的条目。
...
阻塞系统调用被信号打断时，用户态通常看到 `EINTR`，某些带 `SA_RESTART` 的信号可以触发重启语义。当前系统调用分发器在看到 `EINTR` 返回时，会尝试消费一条带 `SA_RESTART` 的用户处理信号，并在写回返回值和推进程序计数器之前构造信号帧。`restart_syscall` 作为兼容入口保留，但当前实现没有在这里维护通用的系统调用重启状态。这个设计使大多数系统调用实现只需返回错误码，不需要直接管理信号帧。
...
#continued-table(
  ...
  (
    ...
    table.cell(fill: warm-fill)[构造信号帧，修改陷阱帧，返回用户处理函数。],
  ),
  kind: "表",
  continuation-kind: "续表",
  align: (left, left, left),
