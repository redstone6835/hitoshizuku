...
#continued-table(
  ...
  (
    ...
    table.cell(fill: stable-fill)[`/dev`、sysfs 文件系统、procfs 文件系统观察设备能力事件后生成视图。],
...
#figure(caption: figure-caption("图", "3-1", [设备模型的单向信息流]))[
  ...
  #layer-card("类型化设备层", "字符、块、网络、RTC 等设备对象保存具体 I/O 语义", fill: handoff-fill)
...
#continued-table(
  ...
  (
    ...
    table.cell(fill: warm-fill)[devtmpfs 文件系统、TTY 层、控制台和通用字符设备适配器。],
...
字符设备能力与 `/dev` 字符设备文件之间存在一层适配。字符设备能力发布的是内核内部能力，devtmpfs 投影器才决定它是否生成 `/dev/console`、`/dev/uart0` 或其它串口节点。这样同一个字符设备可以被投影成多个用户可见入口，也可以先被内核作为启动控制台使用，稍后再补齐用户态节点。
...
#continued-table(
  ...
  (
    ...
    table.cell(fill: soft-fill)[创建 devtmpfs 文件系统超级块，订阅设备能力注册表事件。],
