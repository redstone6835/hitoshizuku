...
#manual-front-section(legend-title)[
  ...
  参考手册的结构讲解部分不会出现真实代码。描述流程或算法时，正文优先使用流程图；当流程图不足以表达条件分支、循环、状态变化或错误处理路径时，正文将使用伪代码。伪代码仅用于说明控制流和数据依赖关系，不与任何具体源文件实现绑定。

  伪代码的默认语言为 C 语言。当正文明确说明需要使用 Rust 语言描述时，则使用 Rust 语言风格的伪代码。例如，在描述 trait 定义或泛型算法等与 Rust 语言紧密相关的设计时，可能会使用 Rust 伪代码；在描述系统调用分发、设备驱动接口等与 C 语言风格更贴近的设计时，使用 C 伪代码。两种伪代码风格服务于同一目的，即表达设计意图而非实现细节，读者应关注伪代码所传达的逻辑，而非其语言形式。
