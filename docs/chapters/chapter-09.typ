...
在第八章中，系统调用接口建立了用户态进入内核的受控入口。本章讨论反方向的问题，即内核如何把一个可执行文件变成真正运行的用户进程。用户态执行环境包括可执行与可链接格式（ELF）装载、解释器处理、地址空间构建、用户栈布置、辅助向量（auxv）、虚拟动态共享对象（vDSO）、初始文件描述符和陷入帧。它们共同决定了动态链接程序、shell 程序、服务进程和普通命令能否按 Linux 兼容预期启动。

用户态支持的难点在于它横跨多个子系统。VFS 负责打开可执行文件和解释器。内存管理负责创建用户虚拟地址空间（`VmSpace`）、提交 `PT_LOAD` 段和映射用户栈。调度层负责把新的地址空间和陷入帧挂到当前任务对象。架构层负责设置入口程序计数器、用户栈指针和返回用户态的状态寄存器。若这些步骤耦合在一个函数里，任何 ABI 细节变化都会牵动多个子系统。我们把装载流程拆成可验证的阶段，让每个阶段只处理自己的对象。
...
#figure(caption: figure-caption("图", "9-1", [用户镜像装载流程]))[
  ...
  #layer-card("陷入帧安装", [设置入口程序计数器、用户栈指针、虚拟内存扩展槽和返回用户态上下文], fill: stable-fill)
]
...
装载完成后，调度层把新的用户虚拟地址空间安装到当前任务扩展槽中，保存执行路径、`argv` 和 `envp` 诊断信息，并构造新的用户陷入帧。旧地址空间和旧用户栈不再参与运行。若执行映像替换发生在 `vfork` 子任务中，替换成功后还需要唤醒被阻塞的父任务，因为子任务已经不再共享父地址空间。
...
用户态装载流程不应直接在一个长函数里修改任务状态。它涉及路径、文件对象、ELF 元数据、解释器、地址空间、用户栈和陷入帧。任何一步失败，都需要回滚已经分配但尚未提交的中间资源。我们因此把执行映像替换分成输入请求、中间镜像和最终装载结果三类对象。输入请求保存路径、`argv`、`envp` 和标志。中间镜像保存解析出的 ELF 元数据、程序头和解释器信息。最终装载结果保存新的用户虚拟地址空间、入口地址、用户栈指针、辅助向量和诊断信息。
...
我们把执行映像替换提交点放在最后。路径打开、shebang 脚本头解析、ELF 读取、解释器读取、`VmSpace` 创建、段提交、栈构造和 vDSO 映射都在新对象上完成。当前任务对象仍然指向旧地址空间。只有所有步骤成功后，`install_exec_image` 才获取必要锁，安装新虚拟内存对象、新陷入帧和诊断信息。旧虚拟内存对象通过引用计数释放。如果安装失败，当前任务仍保留旧地址空间，可以向用户态返回错误。
...
资源失败应当优先返回错误。构造 `argv` 缓冲失败、分配 `VmSpace` 失败、提交段失败、创建用户栈失败，都可以返回 `ENOMEM` 或对应 VFS 错误。用户态压力不应直接导致内核崩溃。只有当内核已经发现自己的基础结构损坏，例如当前任务缺少必须存在的内核栈、陷入帧安装不变量被破坏，才应进入不可恢复处理。这个原则与第八章的异常边界一致。用户输入导致的失败可恢复，内核不变量破坏不可伪装。
...
用户态执行环境把文件系统、内存管理、调度和架构返回路径连接在一起。它需要处理 ELF 细节，也需要维持进程生命周期语义。我们把这个过程拆成路径打开、镜像解析、地址空间构建、栈布局和任务安装几个阶段。

用户态执行环境具备以下创新。

第一是把执行映像替换设计成阶段化事务。路径打开、脚本解析、ELF 校验、解释器装载、地址空间提交、用户栈构造和任务安装被拆成明确阶段。只有 `LoadedUserImage` 完整构造后，才替换当前任务的虚拟内存对象和陷入帧。失败路径保留旧执行状态，成功路径再处理 `CLOEXEC`、`vfork` 唤醒和诊断信息更新。Linux 的 `binfmt_elf` 也有成熟的装载和提交边界，但它服务于更复杂的内核对象体系。我们的实现将这个边界显式化为少数阶段对象，使 Rust 引用计数和错误返回可以自然承担回滚。

第二是 ELF 语义和地址空间策略分离。装载器只解析 ELF、解释器、PIE、程序头和辅助向量语义，真正的 VMA、页表、零填充和权限落实交给 `VmSpace`。xv6 的执行映像替换直接把简单 ELF 段装入新页表，结构清晰但不覆盖动态链接、辅助向量、vDSO 和复杂地址布局。我们的设计保留教学内核那种可理解的阶段顺序，同时支持动态链接程序所需的 ABI 信息。

第三是把用户栈 ABI 作为独立构造对象。`argc`、`argv`、`envp`、辅助向量、随机字节、执行文件名和对齐要求都在同一个栈构造器中处理。字符串先落位，指针后写入，辅助向量最后终止。这个顺序使动态链接器和运行库能够在返回用户态后的第一条路径中读到稳定数据，也让栈布局问题可以集中定位。相较于把栈布置散落在 ELF 装载代码中，独立构造器更适合承接后续 `sigaltstack`、vDSO 和 ASLR 相关扩展。

第四是把资源约束和诊断身份前置到装载模型中。参数长度、环境变量总量、程序头数量、解释器路径、地址范围和段重叠都在装载早期检查。成功后任务对象记录执行路径和必要诊断信息。这样错误输入可以尽早返回，启动失败也能指向路径解析、ELF 解析、地址空间提交、栈布局或任务安装中的具体阶段。这个设计并不追求复刻 Linux 的全部二进制格式装载机制，而是把当前内核最需要的兼容性和可诊断性收束到清晰边界内。

这些设计共同保证用户态程序看到稳定的 Linux 兼容启动环境。内核内部则保持分层。VFS 提供文件，内存管理提供地址空间，调度层保存任务状态，架构层执行最终返回。用户态支持因此成为多个子系统的交汇点，而不是破坏分层的例外路径。它把一个磁盘上的文件转化为可调度、可进入系统调用、可接收信号、可访问文件描述符的进程实体。这个转化过程越清晰，后续用户态兼容和性能优化就越有稳定基础。对于整个内核而言，执行映像替换的可靠性直接决定了用户态生态是否拥有可信起点。它也是从内核初始化走向完整用户态生态的最后一段桥接路径，并为后续信号、终端和网络程序运行提供统一前提，也让用户态问题可以从启动边界开始追踪和复现。稳定的执行环境最终会反过来降低用户态问题的解释成本，并为后续迭代提供直接依据。
