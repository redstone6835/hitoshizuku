...
异常和系统调用必须同时满足两个目标。第一，架构差异必须被限制在清晰边界内。RISC-V64 使用 `ecall`、`sepc` 和 `sstatus`。LoongArch64 使用 `syscall`、`ERA` 和 `PRMD`。寄存器编号、程序计数器推进方式和返回指令都不同。第二，系统调用实现不能被架构细节污染。`openat`、`clone`、`mmap` 和 `rt_sigaction` 等实现只应看到统一的参数数组、当前任务和陷入帧引用。

== 异常入口的职责

异常入口的第一职责是保存现场。来自用户态的异常必须切换到内核栈，并保存用户寄存器、程序计数器、状态寄存器和异常原因。来自内核态的异常可以沿用当前内核栈，但仍需要构造统一的陷入帧。第二职责是区分异常类型。系统调用可以走更短路径，缺页需要进入内存管理，设备中断需要进入中断分发，非法指令和访问错误需要转化为信号或内核故障。第三职责是返回用户态前恢复现场，并在需要时投递信号或执行调度。

#figure(caption: figure-caption("图", "8-1", [异常到系统调用的控制流]))[
  #layer-card("硬件异常入口", [保存寄存器、切换内核栈、读取异常原因和程序计数器], fill: warm-fill)
  #flow-arrow(label: "规范化为陷入帧")
...
平台无关的系统调用分发通过系统调用帧接口（`SyscallFrameOps`）访问陷入帧。架构层提供四个回调。取系统调用号、取六个参数、写返回值和推进程序计数器。这个接口把 ABI 寄存器约定留在架构层，把分发表和系统调用实现留在 general 与 kernel 层。
...
系统调用上下文（`SyscallContext`）把系统调用号、参数、陷入帧和当前任务打包。普通系统调用只需要参数和当前任务。`execve` 和 `sigreturn` 这类特殊调用会重写完整陷入帧，因此可以调用 `finalize_frame` 阻止默认写返回值和推进程序计数器。这个设计避免每个系统调用都理解异常返回细节，同时保留少数特例的控制能力。
...
异常入口必须在极短时间内完成状态规范化。硬件把控制权交给内核时，提供的是架构相关现场。RISC-V64 的异常原因来自 `scause`，异常地址来自 `stval`，返回地址保存在 `sepc`。LoongArch64 的异常原因和子原因来自控制寄存器，返回地址位于 `ERA`，旧特权级信息位于 `PRMD`。如果平台无关层直接读取这些寄存器，系统调用、缺页和信号处理就会到处出现架构分支。我们采用的方式是，在入口层尽快把硬件现场转化为陷入帧（`TrapFrame`）和规范化的异常枚举，再向上层交接。
...
快速路径不能破坏正确性。它只适用于来源明确、上下文简单、返回前不需要额外处理的场景。一旦系统调用过程中发现待处理信号、需要调度、需要重写陷入帧，或者当前任务处于特殊状态，就必须转入完整返回路径。这个设计类似一个短判定链。常见路径走短分支，异常情况回到通用处理。
...
快速路径和完整路径之间的边界必须可证明。我们不把“当前系统调用号是否简单”作为唯一依据，因为简单系统调用也可能遇到待处理信号或调度请求。真正的判断条件来自当前任务和陷入帧状态。若任务仍然运行，未请求调度，没有待处理信号，陷入帧未被特殊系统调用接管，入口可以直接返回。否则走完整路径。这个判定方式比按系统调用白名单更稳健，也避免后续新增系统调用时忘记维护快速路径列表。
...
系统调用数量多，且涉及多个子系统。若所有实现都堆在一个文件中，维护成本会随接口数量快速上升。我们按功能域组织注册。文件系统相关接口在文件系统模块，进程和线程相关接口在进程模块，内存接口在内存模块，信号接口在信号模块，网络接口在套接字模块，系统信息和日志接口在系统模块。启动期 `register_all` 负责把这些模块的实现绑定到统一系统调用表。
...
用户态返回前是内核处理异步事件的天然安全点。系统调用执行过程中，内核可能持有锁、正在修改对象、处于不可睡眠上下文，或者正在复制用户缓冲区。若异步信号在任意位置打断执行，就会破坏这些临界区。我们把信号投递、调度让出和部分任务状态收尾放到返回用户态之前处理。此时系统调用主体已经结束，大部分锁已经释放，陷入帧仍然可修改，用户态还没有恢复执行。

待处理信号的处理需要修改用户上下文。若信号有用户处理函数，内核要在用户栈上写入信号帧，保存旧程序计数器、旧寄存器、旧信号屏蔽字和返回跳板，然后把陷入帧中的程序计数器改为处理函数地址。若信号默认动作为终止，当前任务进入退出流程。若信号被屏蔽，则暂不投递。所有这些动作都需要访问当前任务的信号状态和陷入帧，因此放在返回前最合适。
...
`execve` 和 `rt_sigreturn` 是特殊情况。`execve` 成功后，旧用户上下文已经失效。内核必须安装新地址空间、新用户栈和新入口地址，然后直接返回到新程序。`rt_sigreturn` 则从用户栈取回旧信号帧，恢复到信号发生前的上下文。这两个系统调用都会接管陷入帧，分发器不能再写普通返回值。`finalize_frame` 正是为这类场景提供显式标记。
...
#continued-table(
  ...
  (
    ...
    table.cell(fill: soft-fill)[保存通用寄存器、切换栈、构造陷入帧。],
...
从工程顺序看，系统调用框架应当先保证语义稳定，再优化入口成本。错误码、信号、程序计数器推进和特殊陷入帧接管是正确性条件。快速路径、减少保存寄存器和无锁查表是性能条件。性能优化不能改变正确性条件。我们在文档中把这两类问题分开，是为了后续调优时有明确约束。任何优化只要改变了返回值写入、信号投递或可恢复异常边界，都必须重新审查对应 ABI 语义。

== 工程设计总结

异常处理与系统调用接口处在硬件和内核策略之间。它既要面对寄存器、控制状态寄存器和返回指令，又要为上层提供稳定、简洁的 ABI 分发模型。我们把它拆成异常入口、陷入帧访问契约、表驱动系统调用、返回前收尾和信号调度交接几个部分。

异常处理与系统调用接口具备以下创新。

第一是通过系统调用帧接口（`SyscallFrameOps`）把架构 ABI 收束为少量稳定回调。系统调用实现不需要知道参数来自哪个寄存器，也不需要知道程序计数器应当推进多少字节。Linux 的系统调用入口在各架构下也会保留独立汇编入口和通用 C 分发，但它的工程背景是庞大的成熟 ABI 体系。我们的实现规模更小，因此把差异明确压缩到 `sys_nr`、`sys_args`、`set_sys_ret` 和 `advance_pc` 这几个动作上。相比 xv6 直接在单一架构里读取陷入帧并进入系统调用分派，这个设计更适合同时维护 RISC-V64 和 LoongArch64。

第二是系统调用表采用启动期注册和运行期无锁读取，并通过 `finalize_frame` 处理少数接管返回路径的系统调用。固定表让系统调用号到实现函数的关系集中可查，也避免运行期查找锁。`execve`、`rt_sigreturn` 这类调用需要重写用户上下文，普通返回流程不能再写返回值或推进程序计数器。我们用显式陷入帧接管标记处理这些特例，使特殊控制流只在分发器边界出现，不扩散到所有系统调用实现中。

第三是把快速路径优化放在通用入口链路上。我们没有为某几个简单系统调用硬编码捷径，而是缩短异常入口、现场保存、分发表读取和返回前检查这些公共段。这样所有短系统调用都能共享收益。这个取舍和 Linux 的 vDSO 机制、系统调用入口优化方向相近，但我们当前更关注内核入口本身的统一性，避免在 ABI 层积累大量难以维护的例外。

第四是用户访问窗口和内核可信边界分离清楚。系统调用实现通过统一 helper 读取用户指针，访问错误转化为 `EFAULT`。普通内核路径中的缺页仍然被视为内核故障。这个边界使用户态恶意地址不能扩大为内核崩溃，也防止真正的内核 bug 被错误吞掉。FreeBSD 和 Linux 都有类似的 copyin/copyout 边界，我们在 Rust 内核中把它与异常恢复路径直接绑定，使错误来源更容易推理。

这些设计让异常入口和系统调用接口既贴近硬件，又不把硬件差异扩散到内核主体。平台相关层负责保存和恢复现场。平台无关层负责分发表、错误码和 POSIX 语义。二者在陷入帧契约处交接，形成了清晰的 ABI 边界。对后续章节来说，这个边界还有另一层意义。用户态执行环境、信号、终端和网络都会通过系统调用暴露给用户程序。只要系统调用边界稳定，这些子系统就可以分别演进，而不会把架构 ABI 细节带入自身设计。入口层因此承担了硬件事实和用户态契约之间的长期稳定接口，也承担了性能优化与兼容性之间的协调责任。这个责任贯穿后续所有用户态可见功能，并决定了整套 ABI 的可维护性和问题可解释性，也保证问题能够分层定位和持续演进。后续若继续扩展审计、seccomp 过滤机制或追踪能力，也应当接在这条边界上，而不应侵入具体系统调用实现。
