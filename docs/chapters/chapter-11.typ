...
这个设计让控制台与第三章的设备模型保持一致。UART 串口、virtio 控制台或其它字符设备都可以注册为控制台。devtmpfs 文件系统在启动阶段把选定的字符设备直接绑定为 `/dev/console`。内核日志和用户态文件接口因此可以共享底层字符设备，但两者通过不同入口访问。日志路径不需要进入 VFS，用户态路径也不需要知道全局日志模块。
...
#continued-table(
  ...
  (
    ...
    table.cell(fill: soft-fill)[devtmpfs 文件系统中的控制台节点。],
...
TTY 输入需要处理底层 FIFO 队列和行规程之间的时序。字符设备驱动只报告字节可读。终端控制字符属于 devtmpfs 文件系统和 TTY 兼容层。若直接在中断上下文中执行行规程，就可能触碰 VFS 或 TTY 锁。当前设计把定时器和中断钩子做得很薄，只设置原子请求位。低优先级内核线程在进程上下文中执行 `poll_tty_input`。
...
用户态通过文件描述符访问终端。init 进程启动前，内核通常会为它准备标准输入、标准输出和标准错误。最常见的绑定是 `/dev/console`。这要求 devtmpfs 文件系统能够提供控制台节点，VFS 能够打开它，文件描述符表能够把三个标准文件描述符指向对应文件对象。若其中任一步失败，用户态仍可能运行，但调试和交互会非常困难。
