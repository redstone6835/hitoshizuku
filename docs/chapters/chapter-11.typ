...
这个设计让控制台与第三章的设备模型保持一致。UART 串口、virtio 控制台或其它字符设备都可以注册为控制台。devtmpfs 设备文件系统在启动阶段把选定的字符设备直接绑定为 `/dev/console`。内核日志和用户态文件接口因此可以共享底层字符设备，但两者通过不同入口访问。日志路径不需要进入 VFS，用户态路径也不需要知道全局日志模块。
...
用户态看到的是文件节点。`/dev/console`、`/dev/tty`、串口设备，以及后续可能扩展的伪终端，都需要通过 VFS 的文件操作进入。第四章已经说明 devtmpfs 设备文件系统通过设备文件投影器把设备能力投影为节点。对于控制台，devtmpfs 使用当前选定的字符设备创建固定入口。对于 TTY，节点打开后会构造带行规程的文件操作，使 `read`、`write`、`poll` 和 `ioctl` 都进入终端兼容层。
...
阶段之间需要可切换。早期阶段可能使用固件串口或平台固定 MMIO。设备模型初始化后，真实 UART 串口或 virtio 控制台被枚举并注册为 `CharDevice`。此时内核可以把全局控制台切换到正式设备。VFS 挂载和 devtmpfs 设备文件系统完成后，用户态再通过启动时绑定的 `/dev/console` 使用同一底层设备。这个顺序使日志不会等待完整用户态环境，也使用户态终端不必关心早期输出细节。
...
设备移除时，字符设备会被标记为已移除。全局控制台若仍然指向它，后续写入会得到设备不可用错误。注册新控制台后，全局指针可以替换。这个过程不要求所有旧引用立即消失。需要注意的是，当前 `/dev/console` 是 devtmpfs 中的字符设备节点，代表绑定时的设备对象，并不会因为全局控制台指针替换而自动改写。内核日志路径也不应因为控制台不可用而崩溃。控制台是诊断出口，不是内核运行的必要不变量。这个设计让热插拔和错误设备不会扩大为全局故障。
...
devtmpfs 节点还需要支持轮询。shell 程序和交互程序可能使用 `poll` 或 `select` 等待输入。TTY 文件的 `poll` 实现需要同时观察行缓冲和底层字符设备 FIFO。行缓冲已有完整数据时可以直接报告可读。底层 FIFO 有字节时也要报告可读，使阻塞 `read` 醒来后继续把字节拉进行规程处理。等待者登记落到底层字符设备的等待队列上，输入泵则负责在没有用户态读调用栈时推进控制字符和缓冲状态。若 `poll` 只看行缓冲，规范模式下新到达的串口字节可能没有机会被重新拉进行规程。若 `poll` 只看底层 FIFO，又会绕过回显、控制字符和规范模式缓冲。终端语义必须在 TTY 层完成。
