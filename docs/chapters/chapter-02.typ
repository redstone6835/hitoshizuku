...
本章围绕当前实现中的分层内存管理展开。第一部分说明设计目标和职责边界。第二部分介绍分配器的启动顺序和锁顺序。第三部分讨论启动分配器、伙伴页分配器、地址空间层、Slab 分配器、内核堆和受管堆的分工。第四部分说明用户虚拟地址空间如何把用户地址空间策略保留在平台无关层，并通过架构注入契约访问页表和异常恢复能力。最后一部分总结内存管理子系统的工程创新。
...
#continued-table(
  ...
  (
    ...
    table.cell(fill: handoff-fill)[小对象走 Slab 分配器，大对象走内核堆，受管对象走受管堆。],
...
这些目标决定了内存管理不能只围绕一种算法展开。启动分配器解决的是自举问题。伙伴页分配器解决物理页所有权问题。内核地址空间解决虚拟地址与物理页绑定问题。Slab 分配器和内核堆解决对象分配路径问题。受管堆提供可选的受管对象能力。用户虚拟地址空间面向用户态地址空间，处理 VMA、缺页和页表。每一层都只暴露自己能够稳定承诺的接口。

== 总体分层结构

分配器 Rust 包是分层内核分配器的总入口。它保存启动分配器（`BootAllocator`）、伙伴页分配器（`BuddyAllocator`）、内核地址空间（`KernelAddressSpace`）、内核堆（`KernelHeap`）、Slab 分配器（`SlabAllocator`）、受管分配器（`ManagedAllocator`）、元数据分配器（`MetadataAllocator`）和分配登记表（`AllocationRegistry`）。这些对象共同构成一条从原始启动区间到运行期分配 API 的链路。
...
#figure(caption: figure-caption("图", "2-1", [内存管理子系统分层结构]))[
  ...
  #layer-card("对象分配层", "Slab 分配器处理小对象，内核堆处理大对象，受管堆处理受管对象", fill: stable-fill)
  #flow-arrow(label: "请求带物理后备的虚拟范围")
  #layer-card("内核地址空间层", "内核地址空间管理直接映射、内核和受管三类分配域", fill: handoff-fill)
...
这条分层链路有两个关键性质。其一是所有权向上转化。启动分配器只拥有一段启动期连续区间。伙伴页分配器接管可用物理页。地址空间层将物理页转化为带虚拟地址的后备映射范围（`BackedRange`）。Slab 分配器、内核堆和受管堆再把这些范围解释为对象或缓存。其二是策略向上集中。伙伴页分配器不理解对象大小。地址空间层不理解 Slab 尺寸类别。Slab 分配器不理解用户态 VMA。用户地址空间策略集中在用户虚拟地址空间，不会散落到具体架构页表实现中。

分配器还维护统一的能力查询、统计、审计和回收入口。`capabilities()` 方法暴露 API 版本、页大小、小对象上限、CPU 上限和受管 GC 能力。`stats()` 方法汇总启动分配器、虚拟内存分配域、Slab 分配器、内核堆和受管堆状态。`audit()` 方法可以扫描物理页、虚拟地址、Slab 缓存、内核堆缓存和受管对象。`reclaim()` 方法与 `reclaim_caches()` 方法用于主动释放缓存页。这些入口让内存管理具备运行期可观测性，而不需要外部模块直接读取内部结构。
...
`activate_global` 函数是启动期和运行期的分界。它会检查启动分配器、物理页层、虚拟内存分配域、内核堆和 Slab 分配器是否已经全部就绪，初始化分配登记表，然后封存启动分配器。启动分配器封存后会取出当前游标之后的完整页尾部，通过 `virt_to_phys` 回调转成物理地址，再交还伙伴页分配器。包含已用字节的部分页不会释放，因为其中可能仍保存早期元数据。
...
#continued-table(
  ...
  (
    ...
    table.cell(fill: handoff-fill)[分配登记表早于 Slab 全局状态，Slab 全局状态早于每 CPU 缓存，随后才进入内核堆。],
...
内核地址空间位于伙伴页分配器和对象分配器之间。它解决的问题是拿到物理页后映射到哪里。它内部有三个分配域。`DirectMap` 分配域记录直接映射区。`Kernel` 分配域服务普通内核堆和大对象。`Managed` 分配域服务受管堆。一次带物理后备的分配会经历三步。先在分配域中保留虚拟地址，再向伙伴页分配器申请物理页，最后调用架构层提供的 `map_kernel_heap_range` 回调建立映射。释放时按相反方向撤销映射、归还物理页和释放虚拟区间。

这个层次把三件事隔离开。上层 Slab 分配器和内核堆不需要理解页表。下层伙伴页分配器不需要理解虚拟地址布局。架构层只提供映射和解映射回调，不进入分配器内部锁。后备映射范围是这层交给上层的核心对象，包含分配域、虚拟地址、物理地址、大小和阶数。
...
Slab 分配器和内核堆共同服务 `GlobalAlloc` 全局分配器接口。小请求进入 Slab 分配器，大请求进入内核堆，所有成功分配都会进入分配登记表。登记表记录指针、大小、对齐、分配域、类别和后备信息。释放时先查询登记表，确认所有权，再按记录回到对应分配路径。这个设计让非法释放和重复释放更容易被发现，也为统计和审计提供统一入口。

== 受管堆与精确回收

受管堆是分配器中可选的受管对象区域。它不替代普通内核堆，而是为需要垃圾回收（Garbage Collection, GC）语义的对象提供独立路径。启用时，内核地址空间会为受管分配域分配一段连续虚拟区域，并由受管分配器初始化 GC 控制结构。默认轻量配置是 2 MiB，较大的配置可以使用 64 MiB 级别。

受管堆的核心原则是精确优先。每个受管对象必须携带跟踪描述符（`TraceDescriptor`），其中记录对象大小、对齐、强引用偏移和弱引用偏移。GC 不做保守栈扫描，而是依赖根帧、根句柄、动态精确根槽和自动根提供者。这样可以避免把普通整数误判为指针，也使移动对象、弱引用和终结器更容易保持一致。

GC 路径包含新生代回收、全量回收和显式压力回收。当前实现中存在新生代复制、对象晋升、老年代标记清除和碎片感知压缩决策等机制。受管分配器还提供弱引用、终结器、固定对象、脏卡、记忆集对象和安全点相关状态。外部子系统可以通过精确根注册表注册稳定根槽，也可以安装自动根提供者。

这套受管路径被放在分配器的上层，原因是它需要普通分配能力和地址空间能力已经就绪。它的锁顺序也靠后，避免 GC 在扫描或回收时反向进入物理页热路径。受管堆的价值在于为复杂对象生命周期提供一种可审计的选择，而不是让内核所有对象都进入 GC。
...
`map_anon` 函数和 `map_file` 函数只注册 VMA，不立即分配所有物理页。首次访问时，架构陷入处理器调用平台无关层的 `dispatch_page_fault` 缺页分派入口。这个入口先用缺页解码接口从陷入帧中取出缺页类型、缺页地址和来源特权级。若缺页来自内核态用户访问路径，会先尝试让当前用户虚拟地址空间执行延迟补页，再用 `__ex_table` 异常表修复项把不可修复的用户指针错误转换为可返回的错误。若缺页来自用户态，则交给当前任务的 `VmSpace::handle_fault` 方法。
...
用户指针访问通过用户访问接口集中封装。系统调用路径和装载器调用 `copy_from_user` 函数、`copy_to_user` 函数或 `copy_cstr_from_user` 函数。这些安全包装把内核切片和用户地址传给架构实现。架构侧使用异常表处理访问失败。上层只看到 `Result<_, UserAccessError>` 类型表达式，从而把用户态非法指针归约为 `errno` 错误码，而不会变成不可恢复的内核异常。

== 架构注入契约

内存管理的跨架构复用依赖四组操作接口。用户地址布局接口描述用户页大小、`brk` 基址、`mmap` 区间、默认栈、位置无关可执行文件（Position Independent Executable, PIE）基址、解释器基址和 vDSO 基址。用户页表接口提供用户页表的新建、销毁、映射、解映射、保护、`fork` 克隆、激活和 TLB 失效。用户访问接口提供用户内存复制和用户字符串长度读取。缺页解码接口从陷入帧中提取缺页类型、缺页地址、来源特权级，并尝试进行内核态用户访问修复。
...
#continued-table(
  ...
  (
    ...
    table.cell(fill: stable-fill)[陷入帧结构不进入平台无关层。],
  ),
  kind: "表",
  continuation-kind: "续表",
  align: (left, left, left),
...
本章讨论的内存管理子系统，从启动期的线性分配一直延伸到用户态缺页处理。它的共同主线是把不同阶段的内存问题放在不同层次中处理。启动期关注能否可靠自举。物理页层关注所有权和碎片。地址空间层关注虚拟地址与物理后备的绑定。对象分配层关注热路径延迟和回收。用户地址空间层关注隔离、共享和异常恢复。
...
第四是小对象和大对象采用不同热路径，同时通过分配登记表统一所有权。Slab 分配器面向 2048 字节以内的小对象，使用尺寸类别、位图、每 CPU 缓存和批量补货减少锁竞争。内核堆面向页级对象和大对象，使用范围缓存复用已经具备虚拟地址与物理后备的范围。二者的算法目标不同，但分配结果都会进入分配登记表。释放时必须先通过登记表确认指针来源，再回到对应后端。这个组合让热路径保持专门化，同时让错误释放、重复释放和统计审计具有统一入口。我们没有把所有对象都交给 Slab 分配器，也没有让大对象路径绕过记账。这个取舍保证了性能和所有权检查可以同时存在。

第五是用户地址空间策略集中在用户虚拟地址空间，架构层只实现机械动作。用户虚拟地址空间管理 VMA 集合、常驻页、共享后备、`fork` 系统调用的 COW、`mremap` 系统调用、`mprotect` 系统调用、缺页提交和文件页回写。架构层通过用户页表接口、用户访问接口和缺页解码接口提供页表操作、用户指针访问和缺页解码。这样做的结果是，LoongArch64 和 RISC-V64 可以共用同一套地址空间语义。页表格式、陷入帧和异常表恢复仍保留在各自架构内部。这个边界对系统调用稳定性很重要。用户指针错误会通过用户访问错误返回给系统调用，而不是在平台无关层变成无法恢复的内核异常。

第六是内存管理提供了可选的精确受管堆，同时没有削弱普通内核分配路径。受管堆使用跟踪描述符、根帧、根句柄、动态精确根槽和自动根提供者构造精确根集合。它支持弱引用、终结器、对象晋升、脏卡和多种回收入口。与此同时，普通 `Box` 类型、内核对象、小对象和大对象仍然走 Slab 分配器或内核堆。受管堆作为可选能力存在，使需要复杂生命周期的对象可以使用 GC 语义，也避免把整个内核强制纳入受管运行时。这种设计给后续扩展留下空间，同时保持基础分配路径的可预测性。

从整体看，内存管理子系统的价值不只在于若干算法的组合。更关键的是每个算法都被放在了合适的边界内。启动分配器只负责早期自举。伙伴页分配器只负责物理页。地址空间层只负责虚拟范围和映射协调。Slab 分配器、内核堆和受管堆各自服务不同对象生命周期。用户虚拟地址空间把用户地址空间策略从架构细节中分离出来。边界清晰之后，性能优化和正确性检查才能分别推进，而不会相互牵扯。
