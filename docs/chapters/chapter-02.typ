...
`allocator` 是分层内核分配器的总入口。它保存启动分配器（`BootAllocator`）、伙伴页分配器（`BuddyAllocator`）、内核地址空间（`KernelAddressSpace`）、内核堆（`KernelHeap`）、Slab 分配器（`SlabAllocator`）、受管分配器（`ManagedAllocator`）、元数据分配器（`MetadataAllocator`）和分配注册表（`AllocationRegistry`）。这些对象共同构成一条从原始启动区间到运行期分配 API 的链路。
