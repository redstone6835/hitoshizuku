...
内存管理的跨架构复用依赖四组操作接口。用户地址布局接口描述用户页大小、`brk` 基址、`mmap` 区间、默认栈、位置无关可执行文件（Position Independent Executable, PIE）基址、解释器基址和 vDSO 基址。用户页表接口提供用户页表的新建、销毁、映射、解映射、保护、`fork` 克隆、激活和 TLB 失效。用户访问接口提供用户内存复制和用户字符串长度读取。缺页解码接口从陷阱帧中提取缺页类型、缺页地址、来源特权级，并尝试进行内核态用户访问修复。
