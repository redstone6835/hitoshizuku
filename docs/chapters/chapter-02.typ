...
`allocator` 是分层内核分配器的总入口。它保存启动分配器（`BootAllocator`）、伙伴页分配器（`BuddyAllocator`）、内核地址空间（`KernelAddressSpace`）、内核堆（`KernelHeap`）、Slab 分配器（`SlabAllocator`）、受管分配器（`ManagedAllocator`）、元数据分配器（`MetadataAllocator`）和分配登记表（`AllocationRegistry`）。这些对象共同构成一条从原始启动区间到运行期分配 API 的链路。
...
受管堆是分配器中可选的受管对象区域。它不替代普通内核堆，而是为需要 GC（Garbage Collection，垃圾回收）语义的对象提供独立路径。启用时，内核地址空间会为受管分配域分配一段连续虚拟区域，并由受管分配器初始化 GC 控制结构。默认轻量配置是 2 MiB，较大的配置可以使用 64 MiB 级别。
