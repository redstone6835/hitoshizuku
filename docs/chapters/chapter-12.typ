...
网络系统调用的错误码非常细。`socket` 参数不支持返回 `EAFNOSUPPORT`、`EPROTONOSUPPORT` 或 `EINVAL`。`bind` 地址已用返回 `EADDRINUSE`。`connect` 非阻塞进行中返回 `EINPROGRESS`，已经连接返回 `EISCONN`。`accept` 在监听队列为空且非阻塞时返回 `EAGAIN`。`sendmsg` 遇到对端关闭可能返回 `EPIPE` 并产生 `SIGPIPE`。`recvmsg` 在有序关闭后返回 0。用户态运行库和网络程序会依赖这些细节。

我们在套接字层把协议栈错误转换为错误码。协议栈内部可能使用自己的错误类型，例如不可达、非法、资源耗尽和连接复位。文件操作和系统调用层负责把它们转成 POSIX 错误码。这样协议栈不必直接携带所有 Linux 兼容细节，用户态接口也不会暴露内部错误枚举。错误转换表需要集中维护，避免不同系统调用对同一内部错误返回不同错误码。

`SIGPIPE` 是网络和信号的交叉点。向已经关闭的流式套接字写入，用户态通常收到 `EPIPE`，并可能产生 `SIGPIPE`。若用户忽略或屏蔽 `SIGPIPE`，`write` 返回错误。这个行为需要套接字层在错误发生时调用信号发送接口，同时仍按系统调用语义返回错误码。这里再次体现，网络层不能直接改变用户控制流，只提交信号事件。

地址结构复制也需要兼容。套接字地址由用户态传入，长度可能不足、过长或地址族不匹配。内核必须先安全复制用户结构，再根据地址族解释。`recvfrom` 和 `accept` 还需要把远端地址写回用户缓冲，并处理地址长度指针。用户指针错误返回 `EFAULT`。这部分逻辑依赖第八章的用户访问窗口，也直接影响套接字 ABI 的稳定性。
...
网络子系统连接设备驱动、协议栈、VFS 和用户态套接字。它的核心挑战是保持这些层的边界。驱动只提供链路与包收发。协议栈处理接口、路由和协议状态。VFS 负责文件描述符和 POSIX 套接字语义。设备模型负责生命周期和类型化控制。

网络子系统具备以下创新。

第一是驱动抽象和协议栈适配分离。`NetDriver` 表达硬件能力，`ManagedInterface` 和适配器负责把它接入当前协议引擎。Linux 的网络栈以 `net_device`、NAPI 和 `sk_buff` 为核心，FreeBSD 则围绕 ifnet、mbuf 和协议域展开，二者都把设备和协议栈之间的适配做得很成熟。我们的实现规模更小，因此把边界压缩为驱动、托管接口和套接字句柄三层，使替换协议引擎或新增网卡驱动时不必修改套接字系统调用层。

第二是接口注册表采用读写锁加每接口锁。注册表变化低频，套接字输入输出高频。读写锁允许多条读路径并发进入，不同接口通过各自锁隔离。这个设计在保持分离一致性的同时，避免所有网络操作争用单一全局锁。它没有追求 Linux/FreeBSD 那种高度并行的数据路径，而是先把接口生命周期、路由更新和协议栈串行访问的边界做清楚，适合当前内核规模。

第三是主动轮询与周期轮询结合。周期轮询保证协议状态持续推进。套接字操作后的 `poll_now` 降低回环和短连接路径的延迟，使本机通信不必完全等待定时器推进。这个设计的重点在于提供一个可控的套接字推进点，而不是复刻 NAPI。后续如果引入更完整的中断和网络工作线程，也可以沿这个边界继续演进。

第四是套接字文件描述符统一了网络套接字和 Unix 套接字的用户态入口，同时网络设备控制走类型化请求。二者底层实现不同，但都通过 VFS `FileOps` 暴露 `read`、`write`、`poll`、`ioctl` 和 `close`。MTU、链路状态、MAC 地址和管理启用等设备控制则通过结构化请求表达。这个设计把 POSIX 套接字语义、VFS 文件生命周期和第三章的设备能力模型连接起来，避免让网络设备退化为普通字符流，也避免让套接字层直接解析无类型设备命令。

这些设计使网络子系统可以在当前规模下保持简洁，同时为后续精确套接字唤醒、IPv6、更多驱动和更细粒度路由策略留下空间。网络不是单纯的设备驱动，也不是单纯的文件接口。它位于设备、协议和 POSIX ABI 的交界处，必须用清晰的分层来控制复杂度。
