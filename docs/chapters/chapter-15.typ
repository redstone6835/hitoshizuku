#import "../config.typ": chapter-15-title
...
前十四章已经从启动、内存、设备、文件系统、进程、调度、并发、系统调用、用户态、信号、终端、ELM、网络和原生 ABI 等方向说明了当前内核的主要设计。回到工程实现本身，还需要承认一个更基本的事实。当前系统已经形成了较完整的内核轮廓，也具备跨架构运行、用户态装载、文件系统访问、设备接入和网络通信等能力，但它仍然处在持续收束阶段。很多子系统的边界已经建立，机制也能支撑主要路径，距离长期稳定的通用内核仍有明显差距。
