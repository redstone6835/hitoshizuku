...
`clone` 后还有一个专门的系统调用后交接。新任务创建成功时，父任务的系统调用返回值和程序计数器仍在收尾阶段。若在 `clone` 内部立即切换，父任务的陷入帧可能尚未写好。我们把交接请求记录在 `POST_SYSCALL_HANDOFF` 中，等系统调用分发器完成收尾后再执行一次有界调度。这个设计解决了父子任务首轮运行顺序与陷入帧一致性的冲突。
...
第三是每 CPU 运行队列与单锁子队列组合提供了可扩展性和可推理性。每 CPU 队列避免所有 CPU 争用同一把全局运行队列锁。队列内部用单锁保护多类子队列，降低跨类任务选择和抢占判断的锁序复杂度。有序映射不是理论上最极致的数据结构，但它让插入、删除、迁移和调试都更稳定。这个选择适合当前内核阶段。未来若任务规模增大，可以先替换某个子队列的内部结构，而不改变调度系统外部接口。

第四是重调度被设计成标记和安全点执行的两阶段协议。定时器、唤醒、跨 CPU 请求和 `clone` 后交接都只设置标志或入队。真正的上下文切换发生在系统调用返回、中断返回、主动让出或空闲循环这类安全点。这个边界避免中断在持锁区域直接切走当前任务。系统调用后交接又进一步处理 `clone` 的特殊时序，确保父任务陷入帧收尾后再给子任务运行机会。这里的设计价值不只在性能，更在于它把异步事件对内核不变量的冲击限制在可控位置。

第五是调度系统对多 CPU 能力采用了渐进式结构。代码已经有 CPU 位图、在线 CPU 集、调度域、负载快照、迁移接口和跨 CPU 重调度钩子。当前启动路径主要运行在 CPU0 上，因此这些结构更多承担预留和边界固化作用。这个渐进式设计有实际意义。若等应用处理器启动完成后再重排数据结构，调度器、任务对象、等待队列和 CPU 亲和性都会受到影响。现在先把索引、锁序和拓扑接口固定下来，后续扩展会更平滑。

第六是架构相关操作全部通过钩子注入。调度核心不读取具体 CSR，不解释保存寄存器的布局，也不直接执行某条空闲指令。它只知道如何取得当前 CPU 标识、读取时间戳、请求远端重调度和切换上下文。这个结构使 RISC-V64 与 LoongArch64 共用调度策略，同时允许两者在汇编切换、陷入栈设置和空闲放松动作上各自优化。第一章启动层、第二章内存层和第五章任务执行体都遵循类似模式。调度系统在这里延续了全局的单向依赖原则。
