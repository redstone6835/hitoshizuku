...
第三是是架构相关操作全部通过钩子注入。调度核心不读取具体 CSR，不解释保存寄存器的布局，也不直接执行某条空闲指令。它只知道如何取得当前 CPU 标识、读取时间戳、请求远端重调度和切换上下文。这个结构使 RISC-V64 与 LoongArch64 共用调度策略，同时允许两者在汇编切换、陷入栈设置和空闲放松动作上各自优化。第一章启动层、第二章内存层和第五章任务执行体都遵循类似模式。调度系统在这里延续了全局的单向依赖原则。
