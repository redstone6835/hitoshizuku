...
pub struct PacketSocketFileOps {
    protocol: u16,
    sock_raw: bool,
    bound_ifindex: Option<net::InterfaceId>,
    promiscuous: bool,
    allmulti: bool,
    fanout: Option<(u32, FanoutGroupRef)>,
...
impl PacketSocketFileOps {
    ...
    fn set_bound_ifindex(&self, interface: Option<net::InterfaceId>) {
        unsafe {
            core::ptr::addr_of!(self.bound_ifindex)
                .cast_mut()
                .write(interface)
        };
    }
    ...
    pub fn local_address(&self) -> [u8; 12] {
        ...
        address[2..4].copy_from_slice(&self.protocol.to_ne_bytes());
        let ifindex = self.bound_ifindex.map_or(0, |interface| interface.0);
        ...
        loop {
            ...
            if let Some(frame) = message {
                ...
                if sll_out.len() >= 12 {
                    ...
                    sll_out[2..4].copy_from_slice(&self.protocol.to_ne_bytes());
                    let ifindex = self.bound_ifindex.map_or(0, |interface| interface.0);
    ...
    fn set_allmulti(&self, enabled: bool) {
        unsafe { core::ptr::addr_of!(self.allmulti).cast_mut().write(enabled) };
    }
    ...
    fn set_fanout(&self, group_id: u32, fanout_type: u32) -> Result<(), Errno> {
        ...
        unsafe {
            core::ptr::addr_of!(self.fanout)
                .cast_mut()
                .write(Some((group_id, Arc::clone(&group))));
        }
    ...
    fn fanout_dispatch(&self, frame: Vec<u8>) {
        let Some((_, group)) = self.fanout.as_ref().cloned() else {
...
pub fn packet_socket_deliver(
    ...
    for entry in registry.iter() {
        ...
        if let Some(bound) = socket.bound_ifindex
            && bound != interface
        {
            continue;
        }
        let accepted = match pkttype {
            PACKET_OTHERHOST => socket.promiscuous,
