...
use crate::vfs::cred::{Capability, CapSet, Credentials, Gid, Uid};
...
fn parse_name_namespaces() {
    ...
    assert_eq!(
        xattr::parse_name(b"system.other").unwrap(),
        OtherSystem
    );
    // 非法名
    assert_eq!(xattr::parse_name(b""), Err(VfsError::InvalidArgument));
    assert_eq!(xattr::parse_name(b"user."), Err(VfsError::InvalidArgument));
    assert_eq!(xattr::parse_name(b"user..x"), Err(VfsError::InvalidArgument));
    assert_eq!(xattr::parse_name(b"user.a/b"), Err(VfsError::InvalidArgument));
    // 未知前缀
...
fn acl_parse_encode_roundtrip() {
    ...
    assert_eq!(acl.entries[1], acl::AclEntry { tag: 0x02, perm: 5, id: 1000 });
...
fn acl_validation_rules() {
    ...
    let bad = acl::encode(&PosixAcl {
        entries: vec![acl::AclEntry { tag: acl::ACL_USER_OBJ, perm: 7, id: 0 }],
    });
    assert_eq!(acl::parse(&bad), Err(VfsError::InvalidArgument));
    // 乱序 → 非法
    let bad = acl::encode(&PosixAcl {
        entries: vec![
            acl::AclEntry { tag: acl::ACL_USER_OBJ, perm: 7, id: 0 },
            acl::AclEntry { tag: acl::ACL_OTHER, perm: 0, id: 0 },
            acl::AclEntry { tag: acl::ACL_GROUP_OBJ, perm: 0, id: 0 },
        ],
    });
    assert_eq!(acl::parse(&bad), Err(VfsError::InvalidArgument));
    // 命名条目无 mask → 非法
    let bad = acl::encode(&PosixAcl {
        entries: vec![
            acl::AclEntry { tag: acl::ACL_USER_OBJ, perm: 7, id: 0 },
            acl::AclEntry { tag: acl::ACL_USER, perm: 4, id: 1000 },
            acl::AclEntry { tag: acl::ACL_GROUP_OBJ, perm: 0, id: 0 },
            acl::AclEntry { tag: acl::ACL_OTHER, perm: 0, id: 0 },
        ],
    });
    ...
    // perm 超界 → 非法
    let bad = acl::encode(&PosixAcl {
        entries: vec![acl::AclEntry { tag: acl::ACL_USER_OBJ, perm: 8, id: 0 }],
    });
...
fn acl_enforcement_owner_named_group_other() {
    ...
    ops.setxattr(acl::ACL_ACCESS_XATTR, &acl6(7, Some((1000, 5)), 4, Some((2000, 1)), 4, 0), 0)
        .unwrap();

    // owner：ACL USER_OBJ rwx → 全放行
    assert!(acl::check_access(&cred(0, 0, &[]), &inode, AclCheckKind::Read));
    assert!(acl::check_access(&cred(0, 0, &[]), &inode, AclCheckKind::Write));
    // 命名 user 1000：r-x（读放行、写拒绝）
    assert!(acl::check_access(&cred(1000, 0, &[]), &inode, AclCheckKind::Read));
    assert!(!acl::check_access(&cred(1000, 0, &[]), &inode, AclCheckKind::Write));
    // 命名 group 2000：--x 且 mask r-- → 有效权限 = --x & r-- = 0 → 读拒绝
    assert!(!acl::check_access(&cred(3000, 2000, &[]), &inode, AclCheckKind::Read));
    ...
    ops2
        .setxattr(acl::ACL_ACCESS_XATTR, &acl6(7, None, 0, Some((2000, 4)), 4, 0), 0)
        .unwrap();
    let mut g2 = cred(3000, 3000, &[]);
    g2.groups = vec![Gid(2000)];
    assert!(acl::check_access(&g2, &inode2, AclCheckKind::Read));
    // 组 3000（非成员）：other --- → 拒绝
    assert!(!acl::check_access(&cred(3000, 3000, &[]), &inode, AclCheckKind::Read));
    // DAC_OVERRIDE 能力仍可绕过
...
fn acl_fast_path_without_xattrs() {
    ...
    assert!(acl::check_access(&cred(0, 0, &[]), &inode, AclCheckKind::Read));
    assert!(!acl::check_access(&cred(1, 1, &[]), &inode, AclCheckKind::Read));
}
...
fn default_acl_derives_child_acl_and_mode() {
    ...
    let other = child.entries.iter().find(|e| e.tag == acl::ACL_OTHER).unwrap();
...
fn xattr_permission_model() {
    ...
    assert_eq!(
        xattr::setxattr(&inode, b"user.k", b"v2", xattr::XATTR_CREATE, &cred(0, 0, &[])),
        Err(VfsError::AlreadyExists)
    );
    assert_eq!(
        xattr::setxattr(&inode, b"user.missing", b"v", xattr::XATTR_REPLACE, &cred(0, 0, &[])),
        Err(VfsError::NoData)
    );
    // listxattr 包含已设置的属性
