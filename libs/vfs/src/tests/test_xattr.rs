...
fn acl_validation_rules() {
    ...
    // 命名 uid/gid 0 是合法身份；只有 u32::MAX 表示未定义 ID。
    assert!(acl::parse(&acl6(7, Some((0, 4)), 4, Some((0, 2)), 6, 0)).is_ok());

    // 不携带身份的基础条目必须使用 ACL_UNDEFINED_ID。
    let mut bad_id = acl6(7, None, 4, None, 4, 0);
    bad_id[8..12].copy_from_slice(&0u32.to_le_bytes());
    assert_eq!(acl::parse(&bad_id), Err(VfsError::InvalidArgument));

    // 命名条目反过来不得使用 ACL_UNDEFINED_ID。
    let bad_named = acl6(7, Some((acl::ACL_UNDEFINED_ID, 4)), 4, None, 4, 0);
    assert_eq!(acl::parse(&bad_named), Err(VfsError::InvalidArgument));
}
...
fn acl_enforcement_owner_named_group_other() {
    ...
    // 命名 user 同样受 mask 约束：条目允许执行，但 mask 只允许读。
    assert!(!acl::check_access(
        &cred(1000, 0, &[]),
        &inode,
        AclCheckKind::Exec { is_dir: false }
    ));
...
#[test]
fn acl_named_root_user_and_group_are_enforced() {
    let user_ops = Arc::new(MemXattrOps::new(FileMode::new(0o600)));
    let user_inode = make_inode(0o600, 42, 42, Arc::clone(&user_ops));
    user_inode.mark_has_xattrs();
    user_ops
        .setxattr(
            acl::ACL_ACCESS_XATTR,
            &acl6(6, Some((0, 4)), 0, None, 4, 0),
            0,
        )
        .unwrap();
    assert!(acl::check_access(
    ...
    let group_ops = Arc::new(MemXattrOps::new(FileMode::new(0o600)));
    let group_inode = make_inode(0o600, 42, 42, Arc::clone(&group_ops));
    group_inode.mark_has_xattrs();
    group_ops
        .setxattr(
            acl::ACL_ACCESS_XATTR,
            &acl6(6, None, 0, Some((0, 4)), 4, 0),
            0,
        )
        .unwrap();
    assert!(acl::check_access(
...
#[test]
fn invalid_acl_never_reaches_storage_backend() {
    let ops = Arc::new(MemXattrOps::new(FileMode::new(0o600)));
    let inode = make_inode(0o600, 10, 10, Arc::clone(&ops));
    let malformed = acl::ACL_VERSION.to_le_bytes();

    for name in [acl::ACL_ACCESS_XATTR, acl::ACL_DEFAULT_XATTR] {
        assert_eq!(
            xattr::setxattr(&inode, name, &malformed, 0, &cred(10, 10, &[])),
            Err(VfsError::InvalidArgument)
        );
        assert_eq!(ops.getxattr(name).unwrap(), None);
    }
