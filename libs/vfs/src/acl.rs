...
/// 不携带 uid/gid 的 ACL 条目在 xattr ABI 中必须使用该值。
...
pub fn validate(entries: &[AclEntry]) -> VfsResult<()> {
    ...
    for e in entries {
        ...
        match e.tag {
            ACL_USER_OBJ => {
                if user_obj || e.id != ACL_UNDEFINED_ID {
                    return Err(VfsError::InvalidArgument);
                }
                user_obj = true;
            }
            ACL_USER => {
                named_user = true;
                if e.id == ACL_UNDEFINED_ID {
                    return Err(VfsError::InvalidArgument);
                }
            }
            ACL_GROUP_OBJ => {
                if group_obj || e.id != ACL_UNDEFINED_ID {
                    return Err(VfsError::InvalidArgument);
                }
                group_obj = true;
            }
            ...
            ACL_MASK => {
                if mask || e.id != ACL_UNDEFINED_ID {
                    return Err(VfsError::InvalidArgument);
                }
                mask = true;
            }
            ACL_OTHER => {
                if other || e.id != ACL_UNDEFINED_ID {
                    return Err(VfsError::InvalidArgument);
                }
                other = true;
            }
...
fn acl_check(
    ...
    for e in &acl.entries {
        if e.tag == ACL_USER && e.id == cred.fsuid.0 {
            return allow(e.perm & mask.unwrap_or(0));
        }
    }
