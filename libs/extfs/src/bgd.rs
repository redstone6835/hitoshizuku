...
pub(crate) fn write_desc(
    ...
    if sb.metadata_csum {
        let block_bitmap_csum = bitmap_checksum(backend, sb, desc.block_bitmap)?;
        let inode_bitmap_csum = bitmap_checksum(backend, sb, desc.inode_bitmap)?;
...
fn bitmap_checksum(
    ...
    Ok(crc::update(sb.csum_seed, &bitmap))
}
