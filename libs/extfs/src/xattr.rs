...
/// 原地更新外置 xattr 块前验证其独占所有权。
///
/// ext4 允许多个 inode 共享同一个 xattr 块，`h_refcount > 1` 时必须先做 COW。
/// 当前分配/回写接口还不能把「新块落盘、旧块减引用、inode 换指针」组成一次
/// 可回滚事务，因此这里拒绝共享块修改，绝不原地破坏其它 inode 的属性。
fn ensure_exclusive_mutation(block: &[u8]) -> VfsResult<()> {
    if block.len() < XATTR_HEADER_SIZE
        || u32::from_le_bytes(block[0..4].try_into().unwrap()) != XATTR_MAGIC
    {
...
pub(crate) fn set(
    ...
    let (index, short) = name_index(name)?;
    let old_block = if acl_block != 0 {
        Some(read_block(state, acl_block)?)
    } else {
        None
    };
    let mut entries: Vec<(u8, Vec<u8>, Vec<u8>)> =
        old_block.as_deref().map(parse_block).unwrap_or_default();
    ...
    if let Some(block) = old_block.as_deref() {
        ensure_exclusive_mutation(block)?;
    }
...
mod tests {
    ...
    /// 共享外置块在 COW 事务实现前必须拒绝修改，不能原地改写或释放。
    #[test]
    fn shared_external_block_mutation_is_rejected() {
        let entries = vec![(1u8, b"key".to_vec(), b"value".to_vec())];
        let mut block = build_block(&entries, 4096).unwrap();
        ...
        assert_eq!(ensure_exclusive_mutation(&block), Err(VfsError::Io));
    }
