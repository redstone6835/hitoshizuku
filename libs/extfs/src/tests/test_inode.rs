...
fn file_type_from_mode_regular() {
    assert_eq!(inode::file_type_from_mode(0x8000), vfs::stat::FileType::Regular);
}

/// S_IFDIR (0x4000) 映射为 FileType::Directory。
#[ktest]
fn file_type_from_mode_directory() {
    assert_eq!(inode::file_type_from_mode(0x4000), vfs::stat::FileType::Directory);
}

/// S_IFLNK (0xA000) 映射为 FileType::Symlink。
#[ktest]
fn file_type_from_mode_symlink() {
    assert_eq!(inode::file_type_from_mode(0xa000), vfs::stat::FileType::Symlink);
}
