...
impl VmaSet {
    ...
    pub fn insert(&mut self, area: VmArea) -> Result<(), Errno> {
        ...
        let pos = self.areas.partition_point(|a| a.range.start < area.range.start);
    ...
    pub fn unmap_range(&mut self, range: &Range<usize>) -> Vec<VmArea> {
        ...
        for r in residuals {
            let pos = self.areas.partition_point(|a| a.range.start < r.range.start);
    ...
    fn next_idx(&self, pos: usize) -> Option<usize> {
        let next = pos + 1;
        if next < self.areas.len() { Some(next) } else { None }
    }
