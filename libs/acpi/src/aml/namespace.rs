                ...
                Object::native_method(1, |args| {
                    ...
                    let is_supported = match feature.as_str() {
                        ...
                        "Linux" => {
                            ...
                            warning!("ACPI evaluated `_OSI(\"Linux\")`. This is a bug. Reporting no support.");
                            false
                        }
    ...
    pub fn insert(&mut self, path: AmlName, object: WrappedObject) -> Result<(), AmlError> {
        ...
        match level.values.insert(last_seg, (ObjectFlags::new(false), object)) {
            ...
            Some(_) => {
                 ...
                 * about it. We do warning the user as it does have the potential to break stuff.
                 */
                emergency!("AML name collision: {}. Replacing object.", path);
                Ok(())
            }
        }
    }
