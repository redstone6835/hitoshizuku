...
static void write_histogram(void)
{
    ...
    if (stream == NULL) {
        fprintf(stderr, "profile_observer: open histogram %s failed: %s\n",
    ...
    fputs("\n  ]\n}\n", stream);
    if (fclose(stream) != 0) {
        fprintf(stderr, "profile_observer: close histogram %s failed: %s\n",
                histogram_path, strerror(errno));
    }
}
...
static void write_summary(void)
{
    ...
    if (stream == NULL) {
        fprintf(stderr, "profile_observer: open %s failed: %s\n",
    ...
    fputs("  ]\n}\n", stream);
    if (fclose(stream) != 0) {
        fprintf(stderr, "profile_observer: close %s failed: %s\n",
                summary_path, strerror(errno));
    }
}
    ...
    for (int index = 0; index < argc; ++index) {
        ...
        if (g_str_has_prefix(argument, "socket=")) {
            if (!set_socket_path(argument + strlen("socket="))) {
                fputs("profile_observer: invalid socket path\n", stderr);
                return -1;
            }
            have_socket = true;
            ...
            if (!parse_u64(argument + strlen("period="), 1000, UINT64_MAX,
                           &period_insns)) {
                fputs("profile_observer: invalid period\n", stderr);
                return -1;
            }
            ...
            if (!parse_u64(argument + strlen("stack-bytes="), 0,
                ...
                fputs("profile_observer: invalid stack-bytes\n", stderr);
                return -1;
            }
            ...
            fprintf(stderr, "profile_observer: unknown option: %s\n",
                    argument);
            return -1;
        }
    }
    if (!have_socket) {
        fputs("profile_observer: socket option is required\n", stderr);
        return -1;
    }
    return 0;
}
                                           ...
                                           int argc, char **argv)
{
    ...
    if (!info->system_emulation ||
        ...
        fputs("profile_observer: RISC-V or LoongArch system emulation is required\n",
              stderr);
        return -1;
    }
    ...
    if (output_socket < 0) {
        fprintf(stderr, "profile_observer: socket failed: %s\n",
                strerror(errno));
        return -1;
    }
