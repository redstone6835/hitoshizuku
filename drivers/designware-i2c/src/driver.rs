//! Synopsys DesignWare I2C platform 驱动（最小实现）。
//!
//! 匹配 `snps,designware-i2c`，记录 MMIO/IRQ 资源。
use alloc::sync::Arc;
...
impl Driver {
    ...
    info.has_id("snps,designware-i2c")
    }
}
...
impl PnpDriver for Driver {
    ...
    fn probe(&self, dev: &Arc<PnpDevice>) -> Result<(), PnpError> {
        ...
        log::printk!(
            "[bus-designware-i2c] probe {} phys={:#x} size={:#x} virt={:#x} irq={:?}",
            dev.id,
            phys,
            size,
            virt,
    ...
    fn remove(&self, dev: &Arc<PnpDevice>) {
        if dev.take_driver_data().is_some() {
            log::printk!("[bus-designware-i2c] removed {}", dev.id);
        }
    }
}
