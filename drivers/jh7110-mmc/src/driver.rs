...
use crate::dev::block::{BlockAttributes, BlockClass, BlockDevice, BlockDeviceInit,
    BlockDriver, BlockFeatures, BlockGeometry, BlockLimits};
...
const IDMAC_PHYS_CANDIDATES: [usize; 5] =
    [0x4010_0000, 0x4020_0000, 0x4030_0000, 0x4040_0000, 0x4050_0000];
...
fn alloc_idmac_page() -> Result<(usize, usize), MmcError> {
    ...
    for candidate in IDMAC_PHYS_CANDIDATES {
        ...
        if let Ok(allocation) = KERNEL_ALLOCATOR.allocate_physical(request) {
            ...
            core::mem::forget(allocation); // 永久保留
...
impl JhMmcHost {
    ...
    fn set_clock(&self, hz: u32) -> Result<(), MmcError> {
        ...
        divider = if self.ciu_hz != hz { divider.div_ceil(2) } else { 0 };
...
impl JhMmcHost {
    fn card_init(&self) -> Result<CardInfo, MmcError> {
        ...
        Ok(CardInfo { rca, block_count, block_addressed, is_mmc })
    }
    ...
    fn idmac_prepare(&self, direction: DmaDirection) -> Result<(usize, usize), MmcError> {
        ...
        let dma_base = paddr.checked_add(DMA_PAGE_LAYOUT_DESC).ok_or(MmcError::IoFailed)?;
    ...
    fn write_block(&self, card: &CardInfo, block: u64, buf: &[u8]) -> Result<(), MmcError> {
        ...
        self.write32(REG_BYTCNT, BLOCK_SIZE);
        if let Err(error) =
            self.send_cmd(24, address, CMD_RESP_EXP | CMD_RESP_CRC | CMD_DAT_EXP | CMD_DAT_WR)
        {
            log::warning!(
...
fn ciu_clock(info: &PlatformDeviceInfo) -> Result<Option<(u32, DtbResourceLease)>, PnpError> {
    ...
    Ok(Some((u32::try_from(rate).map_err(|_| {
    ...
    fn queue_bio(&self, mut bio: Bio) -> Result<(), (SubmitError, Bio)> {
        ...
        if block_size != BLOCK_SIZE as usize {
            return Err((SubmitError::InvalidRequest(
                crate::dev::bio::BioReqError::BufferSizeMismatch), bio));
        }
        let total = (bio.range.blocks as usize).checked_mul(block_size);
        let Some(total) = total else {
            return Err((SubmitError::InvalidRequest(crate::dev::bio::BioReqError::TooLarge), bio));
        };
        ...
        for block in 0..bio.range.blocks as u64 {
            ...
            if is_read {
                if self.host.read_block(&self.card, lba + block, &mut scratch).is_err() {
                    failed = true;
                    break;
                }
            }
            ...
            if !is_read && self.host.write_block(&self.card, lba + block, &scratch).is_err() {
                failed = true;
                break;
            }
        }
...
impl PnpDriver for JhMmcDriver {
    fn name(&self) -> &'static str { "platform-jh7110-mmc" }
    ...
    fn probe(&self, dev: &alloc::sync::Arc<PnpDevice>) -> Result<(), PnpError> {
        ...
        if size < 0x300 {
            return Err(PnpError::malformed(PnpResourceKind::Mmio, "mmc reg window too small"));
        }
        ...
        let fifo_data = base
            + if verid < VERID_240A { DATA_OFFSET } else { DATA_240A_OFFSET };
        let idmac_page = alloc_idmac_page().map_err(|_| {
            PnpError::hardware_failure("idmac low-memory page allocation failed")
        })?;
        let host = JhMmcHost { base, fifo_data, fifo_depth, ciu_hz, idmac_page };
        dev.own_resource(dt_provider::lease_pnp_resource(lease, "jh7110-mmc-ciu"))?;
        ...
        host.set_clock(INIT_CLOCK_HZ).map_err(|_| {
            PnpError::hardware_failure("mmc clock init failed")
        })?;
        ...
        let card = match host.card_init() {
            Ok(card) => card,
            Err(first) => {
                log::warning!("[jh7110-mmc] card init attempt 1 failed: {:?}; retrying", first);
                host.reset();
                host.write32(REG_FIFOTH, (2 << 28) | (rx_wm << 16) | tx_wm);
        ...
        // 实机无串口验证探针：MMC（eMMC@16010000）卡初始化成功后，立即把
        // 状态签名写入 eMMC 固定扇区（FAT 分区尾部未分配区），Debian 侧 dd 读取。
        // 之后的内核崩溃（如 PCIe ECAM 越出 MMIO 窗口）不影响本签名。
        if info.fw_path.as_ref().is_some_and(|path| path.contains("16010000")) {
            write_probe_signature(&host, &card, verid, ciu_hz);
        }
        ...
        let block = BlockDevice::new(
            BlockDeviceInit {
                ...
                attributes: BlockAttributes::new(false, false, Some(NonZeroU32::new(1).expect("1")), None),
        ...
        dev.register_function(BlockFunction::with_projection_name_arc(
            &dev.name, &name, Arc::new(block),
        ))?;
        log::printk!("[jh7110-mmc] bound {} phys={:#x} verid={:#x} ciu={} blocks={} -> /dev/{}",
            dev.id, phys, verid, ciu_hz, io.card.block_count, name);
...
/// 把 MMC 探测状态签名写入 eMMC 固定扇区（LBA 221120 起，最多 64 扇区）。
///
/// 用途：实机无串口时的 SSH 侧信道验证。写入内容为纯文本签名，包含主机信息与
/// 卡信息；Debian 侧用 dd if=/dev/mmcblk1 skip=221120 count=64 读取。
fn write_probe_signature(host: &JhMmcHost, card: &CardInfo, verid: u32, ciu_hz: u32) {
    const SIGNATURE_LBA: u64 = 221120;
    const SIGNATURE_SECTORS: u64 = 16;

    let mut text = alloc::string::String::new();
    core::fmt::Write::write_fmt(
        &mut text,
        format_args!(
            "MYGO-MMC-PROBE-SIGNATURE v1\nverid={:#x} ciu={} fifo_depth={}\ncard: mmc={} rca={:#x} blocks={} block_addressed={}\n",
            verid, ciu_hz, host.fifo_depth, card.is_mmc, card.rca, card.block_count, card.block_addressed
        ),
    )
    .ok();

    let mut buffer = [0u8; 512];
    for (index, chunk) in text.as_bytes().chunks(512).enumerate() {
        if index as u64 >= SIGNATURE_SECTORS {
            break;
        }
        buffer.fill(0);
        buffer[..chunk.len()].copy_from_slice(chunk);
        if host.write_block(card, SIGNATURE_LBA + index as u64, &buffer).is_err() {
            break;
        }
    }
}
