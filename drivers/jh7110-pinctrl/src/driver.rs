...
const NO_FUNC_SEL: FuncSel = FuncSel {
    offset: 0,
    shift: 0,
    max: 0,
};
    ...
    fn probe(&self, dev: &alloc::sync::Arc<PnpDevice>) -> Result<(), PnpError> {
        ...
        let children: Vec<(u32, Arc<PinConfigResource>)> = info
            ...
            .map(|nodes| {
                ...
                for node in nodes.iter().filter(|node| {
                    ...
                    let config = match merged.iter_mut().find(|(existing, _)| *existing == phandle)
                    {
        ...
        if let Some(controller_phandle) = controller_phandle {
            ...
            let ctrl_handle = dt_provider::register(
                ctrl_key,
                Arc::new(PinConfigProvider {
                    resource: Arc::clone(&ctrl_resource),
                }),
            )
            .map_err(DtbProviderError::into_pnp_error)?;
            if let Err(err) = dev.own_boxed_resource(dt_provider::provider_pnp_resource_boxed(
                ctrl_handle,
                "jh7110-pinctrl",
            )) {
        ...
        for (phandle, resource) in children {
            ...
            let Ok(handle) = dt_provider::register(key, Arc::new(PinConfigProvider { resource }))
                .map_err(DtbProviderError::into_pnp_error)
            else {
                continue;
            };
            if dev
                .own_boxed_resource(dt_provider::provider_pnp_resource_boxed(
                    handle,
                    "jh7110-pinctrl-state",
                ))
                .is_ok()
            {
                registered += 1;
        ...
        log::printk!(
            "[jh7110-pinctrl] {} registered phandle={:?} states={} mmio={:?}",
