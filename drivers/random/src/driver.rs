//! `/dev/random` 与 `/dev/urandom` 字符设备 ELM 驱动。
//!
//! # 设计概览
//!
...
/// CSPRNG 至少取得一个完整 ChaCha20 密钥长度的熵后才视为安全就绪。
const CRNG_READY_BITS: u64 = 256;
...
// Safety: `state` 只会在成功取得 `flag` 所表示的独占锁后访问，且 `T: Send`
// 保证受保护值能够在线程之间转移。
...
impl<'a, T> core::ops::Deref for SpinLockGuard<'a, T> {
    type Target = T;
    fn deref(&self) -> &T {
        // Safety: 守卫存在期间当前执行上下文持有锁，返回引用不会超过守卫生命周期。
...
impl<'a, T> core::ops::DerefMut for SpinLockGuard<'a, T> {
    fn deref_mut(&mut self) -> &mut T {
        // Safety: 可变借用守卫保证同一时刻只产生一个指向受保护状态的可变引用。
...
impl RandomCore {
    const fn new() -> Self {
        Self {
            ...
            secure_ready: AtomicBool::new(false),
        }
    }
    ...
    fn add_input(&self, data: &[u8], entropy_bits: u64) {
        ...
        let secure_ready = {
            ...
            pool.estimated_entropy_bits >= CRNG_READY_BITS
        };
        if secure_ready && !self.secure_ready.load(Ordering::Acquire) {
            // 必须先把刚获得信用的熵混入 CSPRNG，再发布安全就绪状态；否则
            // 被唤醒的读取者可能观察到 ready，却仍使用旧的弱启动密钥。
            self.reseed_locked();
            self.secure_ready.store(true, Ordering::Release);
        }
        if credited_bits != 0 {
    ...
    fn is_secure_ready(&self) -> bool {
        self.secure_ready.load(Ordering::Acquire)
    }

    /// 等待 CSPRNG 获得足够熵完成安全播种。
    fn wait_for_secure_ready(&self, blocking: bool) -> Result<bool, CharIoError> {
    ...
    /// 安全 CSPRNG 读取；非阻塞且尚未安全播种时返回零长度。
    fn read_secure(&self, buf: &mut [u8], blocking: bool) -> Result<usize, CharIoError> {
        if buf.is_empty() {
            return Ok(0);
        }
        if !self.wait_for_secure_ready(blocking)? {
            return Ok(0);
        }
        Ok(self.read_nonblocking(buf))
    }
...
impl RandomBackend for RandomBackendImpl {
    ...
    fn write(&self, input: &[u8]) -> Result<usize, CharIoError> {
        random_core().write_input(input);
        Ok(input.len())
    }

    fn add_entropy(&self, input: &[u8], entropy_bits: u64) {
        random_core().add_input(input, entropy_bits);
    }
...
pub fn register_builtin_driver() -> Result<RandomBackendHandle, RandomServiceError> {
    seed_from_startup();
    crate::dev::random::register_backend(Arc::new(RandomBackendImpl))
}
...
fn seed_from_startup() {
    ...
    for _ in 0..STARTUP_SOURCE_SAMPLES {
        ...
        let Some(sample) = crate::dev::random_source::sample(&mut buf[pos..]) else {
            break;
        };
    ...
    // 启动提示可以混入状态，但默认不计入熵估值。只有 EntropySource 通过
    // sample_with_credit() 明确声明的熵位数才会解除 `/dev/random` 等待。
...
/// 通过注入的 `EntropySource` 取得时间戳。
#[inline]
fn read_timestamp() -> u64 {
    crate::dev::random_source::timestamp().unwrap_or(0)
}
