//! ARM PrimeCell PL022 SPI platform 驱动（最小实现）。
//!
//! 匹配 `arm,pl022`，记录 MMIO/IRQ 资源。
use alloc::sync::Arc;
...
impl PnpDriver for Driver {
    ...
    fn probe(&self, dev: &Arc<PnpDevice>) -> Result<(), PnpError> {
        ...
        log::printk!(
            "[bus-arm-pl022] probe {} phys={:#x} size={:#x} virt={:#x} irq={:?}",
            dev.id,
            phys,
            size,
            virt,
    ...
    fn remove(&self, dev: &Arc<PnpDevice>) {
        if dev.take_driver_data().is_some() {
            log::printk!("[bus-arm-pl022] removed {}", dev.id);
        }
    }
}
