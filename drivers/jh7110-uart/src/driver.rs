...
impl RegAccess {
    fn from_platform(info: &PlatformDeviceInfo, base: usize) -> Result<Self, PnpError> {
        ...
        if shift >= usize::BITS {
            return Err(PnpError::malformed(PnpResourceKind::Mmio, "invalid reg-shift"));
        }
        if (1usize << shift) < width.bytes() {
            return Err(PnpError::malformed(PnpResourceKind::Mmio, "misaligned regs"));
        }
...
fn uart_clock(info: &PlatformDeviceInfo) -> Result<Option<(u32, DtbResourceLease)>, PnpError> {
    ...
    if rate == 0 {
        return Err(PnpError::malformed(PnpResourceKind::Other("clock"), "zero clock rate"));
    }
    Ok(Some((rate, lease)))
}
...
fn apply_default_pinctrl(dev: &Arc<PnpDevice>, info: &PlatformDeviceInfo) -> Result<(), PnpError> {
    ...
    let Some(reference) = info.dtb_reference_by_name("pinctrl-0", "default")
    ...
    let lease = match dt_provider::acquire_reference(reference) {
        ...
        Err(DtbProviderError::NotReady(_)) => {
            log::warning!("[jh7110-uart] default pinctrl provider not ready; continuing without pin state");
            return Ok(());
        }
        Err(error) => {
    ...
    if let Err(error) = lease.control(DtbResourceRequest::Configure(&[])) {
        log::warning!("[jh7110-uart] default pinctrl configure failed: {:?}", error);
        return Ok(());
    }
    let _ = dev.own_resource(dt_provider::lease_pnp_resource(lease, "jh7110-uart-pinctrl"));
    Ok(())
}
...
impl TxState {
    const fn new() -> Self {
        Self { buf: [0; TX_BUF_SIZE], head: 0, tail: 0, len: 0 }
    }
}
...
impl TxBuffer {
    const fn new() -> Self {
        Self { state: UnsafeCell::new(TxState::new()), lock: AtomicUsize::new(0) }
    }
...
fn register_uart_irq(
    ...
    let handler: Arc<dyn IrqHandler> = Arc::new(Jh7110UartIrqHandler { uart: Arc::clone(&uart) });
    ...
    fn probe(&self, dev: &alloc::sync::Arc<PnpDevice>) -> Result<(), PnpError> {
        ...
        if size != 0 {
            ...
            if last > virt_base + size {
                return Err(PnpError::malformed(PnpResourceKind::Mmio, "reg window too small"));
            }
        }
        ...
        let dev_name = self
            ...
            .into_string();
        let ch = CharDevice::from_arc(info.fw_name.clone(), Arc::clone(&uart) as Arc<dyn CharDriver>);
        ...
        log::printk!("[jh7110-uart] bound {} phys={:#x} shift={} width={} clock={} -> /dev/{}",
            dev.id, phys, regs.shift, regs.width.bytes(), clock_hz.unwrap_or(0), dev_name);
        Ok(())
    }
