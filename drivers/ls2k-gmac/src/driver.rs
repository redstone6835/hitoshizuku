        ...
        if status & (DMA_STATUS_NIS | DMA_STATUS_AIS) != 0 {
            if status & DMA_STATUS_AIS != 0 {
                ...
                if status & (DMA_STATUS_UNF | DMA_STATUS_TU) != 0 {
                    ...
                    // Safety: 同 mask_hardware。
                    unsafe {
                        core::ptr::write_volatile((self.base + DMA_XMT_POLL_DEMAND) as *mut u32, 0)
                    };
                }
            }
            self.mask_hardware();
...
impl GmacMac {
    ...
    fn rx_desc(&self, slot: usize) -> &DmaDesc {
        ...
        unsafe { &*(bytes.as_ptr().add(slot * core::mem::size_of::<DmaDesc>()) as *const DmaDesc) }
    }
    ...
    fn tx_slot_mut(&self, slot: usize) -> &mut [u8] {
        ...
        unsafe { core::slice::from_raw_parts_mut(base.add(slot * TX_BUFFER_SIZE), TX_BUFFER_SIZE) }
    }
    ...
    fn dma_reset(&self) -> Result<(), &'static str> {
        self.write32(
            DMA_BUS_MODE,
            self.read32(DMA_BUS_MODE) | DMA_BUS_MODE_SFT_RESET,
        );
...
impl NetQueuePair for GmacQueue {
    ...
    fn refill_rx_batch(&mut self, batch: &mut RxRefillBatch) -> RxRefillResult {
        ...
        RxRefillResult {
            posted: posted as u16,
            descriptor_starved: !batch.is_empty() && self.rx_pending.iter().all(Option::is_some),
            fatal: None,
        }
    }
    ...
    fn register_irq(
        ...
        match info.register_first_irq_handler(handler) {
            ...
            Err(PlatformIrqRegistrationError::RegistrationFailed { line, err }) => {
                log::printk!(
                    "[ls2k-gmac] failed to register macirq {:?}: {:?}",
                    line,
                    err
                );
...
impl PnpDriver for Ls2kGmacDriver {
    ...
    fn probe(&self, dev: &Arc<PnpDevice>) -> Result<(), PnpError> {
        ...
        .map_err(|_| PnpError::OutOfMemory)?;
        let tx_header_pool = new_netbuf_pool(info.dma_context(), 8, 64, 64, DmaDirection::ToDevice)
            .map_err(|_| PnpError::OutOfMemory)?;
    ...
    fn remove(&self, dev: &Arc<PnpDevice>) {
            ...
            && let Ok(binding) = data.downcast::<Ls2kGmacBinding>()
        {
            ...
            let _ = binding.mac.irq().clear_waker();
