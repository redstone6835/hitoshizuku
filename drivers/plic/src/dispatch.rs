...
/// 选择一个且仅一个在线 PLIC context。
pub(crate) fn select_target_context(
    context_count: usize,
    online_mask: u64,
    fallback_index: usize,
    hwirq: u32,
    mut logical_cpu_at: impl FnMut(usize) -> usize,
) -> Option<usize> {
    if hwirq == 0 || context_count == 0 || fallback_index >= context_count {
        return None;
    }

    let eligible = (0..context_count)
        .map(&mut logical_cpu_at)
        .filter(|&cpu| cpu < u64::BITS as usize && online_mask & (1u64 << cpu) != 0)
        .count();
    if eligible == 0 {
        return Some(fallback_index);
    }

    let target = (hwirq as usize - 1) % eligible;
    (0..context_count)
        .filter(|&index| {
            let cpu = logical_cpu_at(index);
            cpu < u64::BITS as usize && online_mask & (1u64 << cpu) != 0
        })
        .nth(target)
}
...
/// 允许不同 PLIC context 并发处理，同时为驱动卸载提供关闭屏障。
