...
impl Ls2kSpiDriver {
    ...
    fn probe_flash(&self, dev: &Arc<PnpDevice>) -> Result<(), PnpError> {
        ...
        let id = flash.read_id().map_err(|_| PnpError::hardware_failure("spi-nor id read failed"))?;
        ...
        let handle = register_v2(Arc::clone(&flash) as Arc<dyn FlashDevice>, flash as Arc<dyn FlashDeviceV2>)
