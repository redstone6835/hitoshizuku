...
use general::dev::function::{
    DeviceClassId, DeviceFunction, DeviceFunctionInvokeError,
};
...
use general::dev::dt_provider::{
    DtbProviderError, DtbResourceReply, DtbResourceRequest,
};
...
impl Ls2xI2cBus {
    ...
    fn init(&self, clk_rate: u64) {
        ...
        let prescale = (clk / (5 * u64::from(DEFAULT_BUS_HZ))).saturating_sub(1).min(0xffff);
...
impl PnpDriver for Ls2kI2cDriver {
    ...
    fn probe(&self, dev: &Arc<PnpDevice>) -> Result<(), PnpError> {
        ...
        let function: Arc<dyn DeviceFunction> = Arc::new(Ls2xI2cFunction::new(
            Arc::clone(&bus),
            name,
        ));
