//! Cadence USB3 (cdns,usb3) platform 驱动（最小实现）。
//!
//! 匹配 `starfive,jh7110-usb` / `cdns,usb3`，记录 MMIO/IRQ 资源。
use alloc::sync::Arc;
...
impl Driver {
    ...
    info.has_id("starfive,jh7110-usb")
        ||     info.has_id("cdns,usb3")
    }
}
...
impl PnpDriver for Driver {
    ...
    fn probe(&self, dev: &Arc<PnpDevice>) -> Result<(), PnpError> {
        ...
        log::printk!(
            "[bus-cdns-usb3] probe {} phys={:#x} size={:#x} virt={:#x} irq={:?}",
            dev.id,
            phys,
            size,
            virt,
    ...
    fn remove(&self, dev: &Arc<PnpDevice>) {
        if dev.take_driver_data().is_some() {
            log::printk!("[bus-cdns-usb3] removed {}", dev.id);
        }
    }
}
