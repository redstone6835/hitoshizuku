#!/bin/sh
# Validate two repeated profile measurement groups and enforce noise/overhead limits.
set -eu
...
def load(path_text):
    ...
    if data.get("schema") != "mygo.profile" or data.get("schema_version") != 2:
...
report = {
    "schema": "mygo.profile-comparison.v1",
    "accepted": accepted,
    "thresholds": {
        "max_cv_pct": max_cv,
