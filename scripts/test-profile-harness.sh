...
host=$repo/scripts/profile-host.sh
linux_host=$repo/scripts/profile-linux.sh
guest=$repo/scripts/profile-guest.sh
...
if grep -Fq 'profile-resume-ack-' "$guest" || grep -Fq 'profile-resume-probe-' "$guest"; then
...
stage_group_pid=
stage_watch_pid=
trap '[ -z "$stage_watch_pid" ] || kill -KILL "$stage_watch_pid" 2>/dev/null || true; [ -z "$stage_group_pid" ] || kill -KILL "-$stage_group_pid" 2>/dev/null || true; [ -z "$group_pid" ] || kill -KILL "-$group_pid" 2>/dev/null || true; rm -f "$fixture" "$summary_python" "$guest_library" "$deadline_library" "$termination_library" "$container_user_library" "$qemu_library" "$child_pid_file" "$stage_watch_output" /tmp/profile-owner-fixture-mismatch /tmp/profile-owner-fixture-group /tmp/profile-owner-fixture-stage; rm -rf "$summary_dir" "$stage_root" "$slow_awk_dir" "$natural_dir" "$early_dir" "$qemu_diag_dir" "$qemu_fake_bin"' EXIT INT TERM
printf '#!/bin/sh\nsleep 30\n' >"$slow_awk_dir/awk"
...
if PROFILE_DURATION_MS=1 PROFILE_BOOT_MODE=linux PROFILE_CAPTURE=1 "$host" >/dev/null 2>&1; then
    echo "validation fixture: Linux accepted Hitoshizuku guest capture" >&2
    exit 1
fi
...
printf '%s %s %s\n' "$stage_group_pid" "$stage_start" fixture-stage \
    >/tmp/profile-owner-fixture-stage
...
rm -f /tmp/profile-owner-fixture-stage
wait "$stage_watch_pid"
stage_watch_pid=
...
set -- $rest
start_ticks=${20}
printf '%s %s %s\n' "$group_pid" "$start_ticks" fixture-group >/tmp/profile-owner-fixture-group
...
group_pid=
printf '%s %s %s\n' "$$" 1 fixture-mismatch >/tmp/profile-owner-fixture-mismatch
...
assert data["schema"] == "mygo.profile"
...
echo "profile profile harness fixtures: ok"
