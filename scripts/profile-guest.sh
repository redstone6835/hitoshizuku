...
tool_mount=${PROFILE_TOOL_MOUNT:-/tmp/profile-tools}
...
print_workload_plan() {
    ...
    cat <<EOF
schema=mygo.profile-workload.v2
arch=$profile_arch
...
prepare_target() {
    ...
    mkdir -p "$target_mount" || {
        echo "profile runner: unable to create profile target directory" >&2
        return 1
    }
    ...
    case "$profile_target_fs" in
        extfs)
            if awk -v target="$target_mount" '$2 == target { found = 1 } END { exit !found }' \
                ...
                echo "profile runner: formal profile target has an unexpected mount" >&2
                return 1
            fi
            ...
            case "$workload_fs" in
                ext2|ext3|ext4) ;;
                *) echo "profile runner: formal profile requires an ext filesystem" >&2; return 1 ;;
            esac
        ...
        tmpfs)
            if awk -v target="$target_mount" \
                ...
                echo "profile runner: unable to mount profile target tmpfs" >&2
                return 1
            fi
...
# The Hitoshizuku-specific task snapshot avoids /proc/PID/stat's VMA accounting path.
...
# Linux does not expose Hitoshizuku's atomic task snapshot. One awk process reads all
...
terminate_group() {
    ...
    # it.  Do not scan /proc here to distinguish that state: Hitoshizuku procfs can
...
set_control() {
    ...
    control=/mnt/run/profile-control-$token
    [ -p "$control" ] || {
...
controller_status() {
    ...
    IFS= read -r controller_status_value \
        <"/tmp/profile-controller-phase-$token" 2>/dev/null || true
...
release_workload() {
    ...
    gate=/mnt/run/profile-gate-$token
...
capture_controller() {
    ...
    workload_pid=$4
    workload_start=$5
    controller_gate=/mnt/run/profile-gate-$token
    controller_gate_released=$controller_gate.released
    controller_phase_file=/tmp/profile-controller-phase-$token
    exec 8<>"$control" || {
...
run_profile() {
    ...
    if [ "$skip_prebuild" -eq 1 ]; then
        ...
        if ! chroot /mnt /bin/bash -lc \
            'export PATH=/root/.cargo/bin:/usr/local/bin:/usr/bin:/bin:/sbin:/usr/sbin HOME=/root RUSTUP_HOME=/root/.rustup CARGO_HOME=/root/.cargo CARGO_NET_OFFLINE=true; cd /work/tgoskits; cargo build -p tg-xtask'; then
    ...
    rm -f "/tmp/profile-controller-phase-$token"
