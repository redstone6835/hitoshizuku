#!/bin/sh
# Run one cold profiling window under QEMU.
set -eu
...
container_image=${PROFILE_CONTAINER_IMAGE:-}
[ -n "$container_image" ] || {
    echo "PROFILE_CONTAINER_IMAGE must name a build image" >&2
    exit 2
}
...
observer_plugin=${PROFILE_QEMU_PLUGIN:-"$repo/build/qemu-plugins/profile_observer.so"}
...
run_dir=$(mktemp -d "${PROFILE_RUN_ROOT:-/tmp}/profile-${safe_label}.XXXXXX")
stage=$(mktemp -d "${PROFILE_RUN_ROOT:-/tmp}/profile-tools.XXXXXX")
...
cp "$repo/scripts/profile-guest.sh" "$stage/run.sh"
...
set -- "$@" -v "$run_dir":/run -v "$base_dir":/base:ro "$container_image" \
    ...
    -name guest=profile,debug-threads=on \
...
if [ "$observer_enabled" -eq 1 ]; then
    set -- python3 "$repo/scripts/qemu_profile_daemon.py" capture \
        ...
        --workload profile-arceos-helloworld \
        --vcpu-count "$smp" \
...
if [ "$capture_started" -eq 1 ]; then
    ...
    python3 "$repo/scripts/analyze-profile-syscalls.py" \
