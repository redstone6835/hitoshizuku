#!/usr/bin/env python3
"""分析一次已完成的 RISC-V profile 指令与 TCG task-clock 画像。
...
ANALYSIS_SCHEMA = "mygo.riscv-profile-instruction-analysis.v1"
CACHE_VERSION = 4
...
def _arguments(argv: Sequence[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument("run_dir", type=Path, help="已完成的 profile 运行目录")
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        print(f"analyze-riscv-profile-instructions.py: {error}", file=sys.stderr)
        return 2
