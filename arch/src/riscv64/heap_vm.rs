...
use general::{MapError, PagingArch, find_leaf, unmap_range_entries};
use spin::Mutex;
...
fn existing_kernel_tlb_flush_plan(
    ...
    while current_vaddr < end_vaddr {
        let (level, _, _) = find_leaf::<Riscv64Paging>(root_vaddr, current_vaddr, phys_to_virt)?;
        let leaf_size = Riscv64Paging::leaf_page_size(level).ok_or(MapError::UnsupportedLevel)?;
