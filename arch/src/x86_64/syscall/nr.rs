...
//!
//! 编号由 Linux UAPI `arch/x86/include/generated/uapi/asm/unistd_64.h`
//! 生成，只描述原生 64 位 ABI；不包含 x32 兼容号段。
...
pub const SYS_SYNC_FILE_RANGE: Option<usize> = Some(277);
...
/// kernel 的语义名沿用 asm-generic 的 fstatat 命名。
pub const SYS_FSTATAT: usize = SYS_NEWFSTATAT;

/// x86_64 使用原生 64 位 time ABI，没有 asm-generic 的独立 time64 入口。
...
/// x86_64 的 277 是 sync_file_range，参数顺序不同于 sync_file_range2。
...
// Hitoshizuku 私有入口，位于当前 Linux x86_64 公开号段之后、x32 号段之前。
pub const SYS_ELM_CTL: usize = 509;
...
mod tests {
    ...
    #[test]
    fn representative_linux_numbers_match_x86_64_uapi() {
    ...
    #[test]
    fn absent_compat_entries_are_not_assigned_reserved_numbers() {
        assert_eq!(SYS_CLOCK_GETTIME64, None);
        assert_eq!(SYS_FUTEX_TIME64, None);
        assert_eq!(SYS_SYNC_FILE_RANGE, Some(277));
        assert_eq!(SYS_SYNC_FILE_RANGE2, None);
        assert_eq!(SYS_RISCV_FLUSH_ICACHE, None);
    }

    #[test]
    fn private_numbers_do_not_overlap_native_linux_uapi() {
    ...
    #[test]
    fn seccomp_metadata_uses_native_x86_64_numbers() {
        assert_eq!(AUDIT_ARCH, 0xc000_003e);
        assert_eq!(LINUX_SYSCALL_ABI.strict_allow, [0, 1, 60, 15]);
        assert!(LINUX_SYSCALL_ABI.strict_allows(SYS_RT_SIGRETURN));
        assert!(!LINUX_SYSCALL_ABI.strict_allows(139));
    }
}
